...

{
 ...
 for(i=0;i<NB_OF_STACKS;i++)
    {
     ...
     if ((stk_tbl[i].size=KBytes_To_Wam_Words(*(stk_tbl[i].p_def_size)))==0)
 ...
 heap_actual_start=Global_Stack;   /* must be changed to store global info */
                                   /* see for instance the debugger.       */
...
/*-------------------------------------------------------------------------*/
void Set_Heap_Actual_Start(WamWord *new_heap_actual_start)
...
/*-------------------------------------------------------------------------*/
void Execute_Directive(int pl_file,int pl_line,Bool is_system,CodePtr proc)
