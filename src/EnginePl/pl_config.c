...
/*
 * pl_reg_bank is a ptr (i.e. WamWord *).
 * To have an array (ie. WamWord [size]) define PL_REG_BANK_AS_ARRAY
 * Could be activated if M_TRAMPOLINE_REG_BANK is not defined
 * (ensure trampoline.S and mappers are ok for an array).
 */
#if 0
#define PL_REG_BANK_AS_ARRAY
#endif
...
#define MSG_GENERATED_FILE \
  "/* this file is generated by pl_config.c - do not edit */\n"
...
main(void)
{
  ...
  if ((fwam_regs = fopen(FILE_WAM_REGS_H, "wb")) == NULL)
    Fatal_Error("cannot open %s", FILE_WAM_REGS_H);

  fprintf(fwam_regs, MSG_GENERATED_FILE);
  fprintf(fwam_regs, "\n#include \"gp_config.h\"\n\n");


  if ((fwam_stacks = fopen(FILE_WAM_STACKS_H, "wb")) == NULL)
    Fatal_Error("cannot open %s", FILE_WAM_STACKS_H);

  fprintf(fwam_stacks, MSG_GENERATED_FILE);

  if ((fg_cst = fopen(FILE_GPROLOG_CST_H, "wb")) == NULL)
    Fatal_Error("cannot open %s", FILE_GPROLOG_CST_H);
#if 0
  fprintf(fg_cst, MSG_GENERATED_FILE);
#endif
...
#ifndef NO_USE_REGS
         "%d gcc global register variables (GRV)\n", nb_of_used_mach_regs
#elif defined(M_TRAMPOLINE_REG_BANK)
         "%d callee-saved register set in the trampoline\n", nb_of_used_mach_regs
#else
	 "none"
#endif
    );
...
#if 0
  fprintf(fg_cst, "/* --- end of generated part --- */\n");
#endif
  ...
  if (nb_of_used_mach_regs)
	...
	printf("%s (%s)  ", used_mach_reg[i].mach_reg_name,
	       used_mach_reg[i].pl_reg_name);
      printf("\n");
    }
  else
    printf("none\n");
...
Write_GProlog_Cst(void)
{
  ...
  fprintf(fg_cst, "#define __GNU_PROLOG__        \t%d\n", major);
  fprintf(fg_cst, "#define __GPROLOG__           \t%d\n", major);
  fprintf(fg_cst, "#define __GPROLOG_MINOR__     \t%d\n", minor);
  fprintf(fg_cst, "#define __GPROLOG_PATCHLEVEL__\t%d\n", patch_level);
  fprintf(fg_cst, "#define __GPROLOG_VERSION__   \t%d\n",
          major * 10000 + minor * 100 + patch_level);

  fputc('\n', fg_cst);

  fprintf(fg_cst, "#define PROLOG_DIALECT  \t\"" PROLOG_DIALECT "\"\n");
  fprintf(fg_cst, "#define PROLOG_NAME     \t\"" PROLOG_NAME "\"\n");
  fprintf(fg_cst, "#define PROLOG_VERSION  \t\"" PROLOG_VERSION "\"\n");
  fprintf(fg_cst, "#define PROLOG_DATE     \t\"" PROLOG_DATE "\"\n");
  fprintf(fg_cst, "#define PROLOG_COPYRIGHT\t\"" PROLOG_COPYRIGHT "\"\n");
...
Write_C_Compiler_Info(void)
{
  ...
  fprintf(fwam_regs, "#define CFLAGS_REGS\t\t\"");
#ifndef NO_USE_REGS
  for (i = 0; i < nb_of_used_mach_regs; i++)
    {
...
#pragma warning(disable : 4474)	/* too many args for fprintf */
      ...
      fprintf(fwam_regs, CFLAGS_PREFIX_REG, used_mach_reg[i].mach_reg_name);
      fputc(' ', fwam_regs);
    }
#endif
  fputs("\"\n", fwam_regs);
...
Generate_Archi(void)
{
  ...
  if ((g = fopen(FILE_WAM_ARCHI_H, "w")) == NULL)
    Fatal_Error("cannot open %s", FILE_WAM_ARCHI_H);

  fprintf(g, MSG_GENERATED_FILE);
...
Generate_Regs(FILE *f, FILE *g)
{
  char *p1, *p2;
  char str[STR_LENGTH];
  char str_base[32 + STR_LENGTH] = ""; /* 32 is enough but avoid gcc sprintf warning */
  char *used_regs[] = M_USED_REGS;
  char **p = used_regs;
  static RegInf reg[15][50];	/* static: referenced after function return */
  static int nb_reg[NB_REG_PRIOR]; /* one by priority (static => all init with 0) */
  RegInf *dp;
  int total_nb_reg = 0;
  int nb_not_alloc = 0;
...
#endif
      fprintf(fwam_regs, "#define MAP_REG_BANK\t\t\"%s\"\n\n", *p);
      p++;
    }
        ...
        if (*p)
          {
            ...
            fprintf(g, "register %s\t\t%-3s asm (\"%s\");\n",
                    dp->type, dp->name, *p);
            fprintf(fwam_regs, "#define MAP_REG_%-10s\t\"%s\"\n", dp->name, *p++);
            if (!*p)
              fprintf(g, "\n\n");
          }
        else
#endif
          {
            ...
            fprintf(fwam_regs, "#define MAP_OFFSET_%-6s\t((%s%d)*%d)\n",
...
#ifdef PL_REG_BANK_AS_ARRAY	/* pl_reg_bank restored anyway */
...
#define Mk_Tag_Mask(x)							\
  (tag_size_high == 0) ? ((PlULong) (x)) :				\
    ((((PlULong) (x) >> tag_size_low) << (value_size + tag_size_low)) |	\
     ((x) & ((1 << tag_size_low) - 1)))
  ...
  fprintf(fg_cst, "#define PL_MIN_INTEGER\t\t%" PL_FMT_d "\n",  min_integer);
  fprintf(fg_cst, "#define PL_MAX_INTEGER\t\t%" PL_FMT_d "\n",  max_integer);
  fputc('\n', fg_cst);
  ...
  fprintf(g, "#define Tag_Mask_Of(w)\t\t((PlULong) (w) & (TAG_MASK))\n");
  ...
  for (i = 0; i < nb_tag; i++) {
    fprintf(g, "#define TAG_%s_MASK\t\t(WamWord) %#" PL_FMT_x "\n",
  ...
  for (i = 0; i < nb_tag; i++) {
    ...
    fprintf(fg_cst, "#define PL_%-10s \t\t%-2d\n", tag[i].name, tag[i].value);
  }
 ...
 */
  ...
  fprintf(g, "\n");
  fprintf(g, "#define UnTag_Long_Int(w) \t(((PlLong) ((PlULong) (w) << %d)) >> %d)\n",
...
Generate_Stacks(FILE *f, FILE *g)
{
  ...
  fprintf(fwam_stacks, "#define NB_OF_STACKS \t\t%d\n\n", nb_stack);
  ...
  for (i = 0; i < nb_stack; i++)
    {
      ...
      *str = toupper(*str);
      ...
      fprintf(fwam_stacks, "#define %s_Stack       \t(pl_stk_tbl[%d].stack)\n", str, i);
      fprintf(fwam_stacks, "#define %s_Size        \t(pl_stk_tbl[%d].size)\n", str, i);
      fprintf(fwam_stacks, "#define %s_Offset(adr) \t((WamWord *)(adr) - %s_Stack)\n",
              str, str);
      fprintf(fwam_stacks, "#define %s_Used_Size   \t%s_Offset(%s)\n\n", str, str,
              stack[i].top_macro);
    }

  fprintf(fwam_stacks, "\n#define Stack_Top(s)       \t(");
  for (i = 0; i < nb_stack - 1; i++)
    fprintf(fwam_stacks, "((s) == %d) ? %s : ", i, stack[i].top_macro);

  fprintf(fwam_stacks, "%s)\n", stack[nb_stack - 1].top_macro);


  fprintf(fwam_stacks, "\ntypedef struct\n");
  fprintf(fwam_stacks, "{\n");
  fprintf(fwam_stacks, "  char *name;\n");
  fprintf(fwam_stacks, "  char *desc;\n");
  fprintf(fwam_stacks, "  char *env_var_name;\n");
  fprintf(fwam_stacks, "  PlLong *p_def_size;  \t/* used for fixed_sizes */\n");
  fprintf(fwam_stacks, "  PlLong default_size; \t/* in WamWords */\n");
  fprintf(fwam_stacks, "  PlLong size;         \t/* in WamWords */\n");
  fprintf(fwam_stacks, "  WamWord *stack;\n");
  fprintf(fwam_stacks, "}InfStack;\n\n\n");


  fprintf(fwam_stacks, "#ifdef ENGINE_FILE\n\n");

  fprintf(fwam_stacks, "    /* these variables can be overwritten by top_comp.c (see stack size file) */\n");

  for (i = 0; i < nb_stack; i++)
    fprintf(fwam_stacks, "PlLong pl_def_%s_size;\n", stack[i].name);
  fprintf(fwam_stacks, "PlLong pl_fixed_sizes;\n\n");

  fprintf(fwam_stacks, "InfStack pl_stk_tbl[] =\n{\n");
  ...
  for (i = 0; i < nb_stack; i++)
    {
      ...
      fprintf(fwam_stacks, " { \"%s\", %s, \"%sSZ\", &pl_def_%s_size, %d, 0, NULL }%s",
  ...
  fprintf(fwam_stacks, "\n#else\n\n");
  for (i = 0; i < nb_stack; i++)
    fprintf(fwam_stacks, "extern PlLong pl_def_%s_size;\n", stack[i].name);
  fprintf(fwam_stacks, "extern PlLong pl_fixed_sizes;\n\n\n");
  fprintf(fwam_stacks, "extern InfStack pl_stk_tbl[];\n");
  fprintf(fwam_stacks, "\n#endif\n");
