...
Read_Identifier(char *s, int fail_if_error, char **end)
{
  ...
  *end = s;
  ...
  if (!isspace(**end))
    Fatal_Error("Syntax error: space expected after identifier in: %s", save_str);
...
Read_String(char *s, char **end)
{
  ...
  if (*s != '"')
    Fatal_Error("Syntax error: string expected in: %s", save_str);
...
Read_Integer(char *s, char **end)
{
  ...
  x = (int) strtol(s, end, 0);
  if (*end == s)
    Fatal_Error("Syntax error: integer expected in: %s", save_str);
  if (!isspace(**end))
    Fatal_Error("Syntax error: space expected after integer in: %s", save_str);
  **end = '\0';
  return x;
#else
  while (isspace(*s))
...
Generate_Regs(FILE *f, FILE *g)
{
  char *p1, *p2;
  static char str[STR_LENGTH];
  char str_base[32 + STR_LENGTH] = ""; /* 32 is enoungh but to avoid gcc sprintf warning */
      ...
      if (strcmp(p1, "stack") == 0)
        {
          ...
          i = Read_Integer(p2 + 1, &p2);
          stack[nb_stack].def_size = (i * 1024 + sizeof(PlLong) - 1) / sizeof(PlLong);
