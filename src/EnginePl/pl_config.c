...
#define Mk_Tag_Mask(x) ((((unsigned long) (x) >> tag_size_low) << (value_size + tag_size_low)) | ((x) & ((1 << tag_size_low) - 1)))
  ...
  max_integer = (1L << (WORD_SIZE - tag_size - 1)) - 1;
  min_integer = -max_integer - 1;

  fprintf(fg_c, "#define PL_MIN_INTEGER\t\t%ldL\n", min_integer);
  fprintf(fg_c, "#define PL_MAX_INTEGER\t\t%ldL\n", max_integer);
  fputc('\n', fg_c);
...
Generate_Stacks(FILE *f, FILE *g)
{
  ...
  for (i = 0; i < nb_stack; i++)
    {
      ...
      *str = toupper(*str);
      ...
      fprintf(g, "#define %s_Stack       \t(pl_stk_tbl[%d].stack)\n", str, i);
      fprintf(g, "#define %s_Size        \t(pl_stk_tbl[%d].size)\n", str, i);
  ...
  for (i = 0; i < nb_stack; i++)
    fprintf(g, "long pl_def_%s_size;\n", stack[i].name);
  fprintf(g, "long pl_fixed_sizes;\n\n");

  fprintf(g, "InfStack pl_stk_tbl[] =\n{\n");
  ...
  for (i = 0; i < nb_stack; i++)
	...
	*p1 = toupper(*p1);
      ...
      fprintf(g, " { \"%s\", \"%sSZ\", &pl_def_%s_size, %d, 0, NULL }%s",
