...
Pl_Call_Compiled(CodePtr codep)
{
...
#if !defined(MAP_REG_BANK) && !defined(M_TRAMPOLINE_REG_BANK)
  ...
  register WamWord *rb asm("s6") = pl_reg_bank;
  pl_ensure_reserved = (WamWord *) rb; /* to avoid gcc warning */
...
#if defined(M_win64)
  asm("adrp x20, " UN "pl_reg_bank");
  asm("add  x20, x20, " UN ":lo12:pl_reg_bank");
  asm("ldr  x20, [x20]");
...
#ifdef _WIN32
  asm volatile ("movq %0,%%r12" : : "m"(pl_reg_bank) : "r12");
#else
...
#endif /* !defined(MAP_REG_BANK) && !defined(M_TRAMPOLINE_REG_BANK) */
