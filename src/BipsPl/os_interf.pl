...
'$file_prop'(absolute_file_name(AbsolutePathName),PathName):-
	'$call_c_test'('File_Prop_Absolute_File_Name_2'(AbsolutePathName,
                                                        PathName)).
...
exec(Cmd,StreamIn,StreamOut,StreamErr,Pid):-
	set_bip_name(exec,5),
        (nonvar(Pid)       -> '$pl_err_type'(variable,Pid)
                           ;   true),
	'$sys_var_write'(0,0),
	'$sys_var_set_bit'(0,0),
	'$exec'(Cmd,StreamIn,StreamOut,StreamErr,Pid).
...
select(Reads,ReadyReads,Writes,ReadyWrites,TimeOut):-
	...
	'$call_c_test'('Select_5'(Reads,ReadyReads,
                                  Writes,ReadyWrites,TimeOut)).
