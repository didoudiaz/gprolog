...
static char unget_tok_buffer[LEXER_BIG_BUFFER];
 ...
 * Except this case, a boolean tok_present + save pl_token.line/col
 * (for error reporting) to restore then after Unget_Token is enough.
 * NB: Unget_Token is called very ofter and MUST be fast.
 * We not copy too large blocks of memory but we swap buffers instead.
 */
...
/*-------------------------------------------------------------------------*
 * PARSER_SUPP_INITIALIZER                                                 *
 *                                                                         *
 *-------------------------------------------------------------------------*/
PL_INITIALIZER(Parser_Supp_Initializer)
{
...
Read_Next_Token(Bool comma_is_punct)
{
  ...
  else if ((err_msg = Pl_Lexer_Token(pstm_i, comma_is_punct)) != NULL)
    Parse_Error(err_msg);
}
 ...
 * parse_end_of_term controls the end of term (see parser_supp.h).         *
 ...
 * Since the Pl_Lexer_Token() only consumes necessary characters, the      *
 ...
 * (thus the next call to Pl_Lexer_Token() will return TOKEN_IMMEDIAT_OPEN)*
 * We use Pl_Lexer_Peek_Char() instead of Pl_Stream_Peekc() only to deal   *
...
Parse_Term(int cur_prec, int context, Bool comma_is_punct)
{
  ...
  switch (pl_token.type)
    ...
    a_name:
      bracket = (Pl_Lexer_Peek_Char(pstm_i, TRUE) == '(');
...
Pl_Read_Atom(StmInf *pstm)
{
  ...
  if ((err_msg = Pl_Lexer_Next_Atom(pstm)) != NULL)
    {
...
Pl_Read_Number(StmInf *pstm, Bool integer_only)
{
  ...
  if ((err_msg = Pl_Lexer_Token(pstm, COMMA_ANY)) != NULL)
    {
...
Pl_Read_Token(StmInf *pstm)
{
  ...
  if ((err_msg = Pl_Lexer_Token(pstm, FALSE)) != NULL)
    {
      Pl_Set_Last_Syntax_Error(pl_atom_tbl[pstm->atom_file_name].name,
			       pl_token.line, pl_token.col, err_msg);
      return NOT_A_WAM_WORD;
