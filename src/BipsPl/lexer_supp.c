...
static void Lexer_Number(StmInf *pstm, Bool integer_only);

static void Lexer_Quoted(StmInf *pstm);

static int Lexer_Quoted_Char(StmInf *pstm, Bool convert, int c0, Bool no_escape);
 ...
 *-------------------------------------------------------------------------*/
int
Pl_Lexer_Peek_Char(StmInf *pstm, Bool convert)
{
  int c_look;
 ...
 * Unget_Last_Char if necessary (see Lexer_Number). Thus after a pl_token  *
 * has been read Pl_Stream_Peekc() will return the character directly      *
 * following this pl_token.                                                *
 *-------------------------------------------------------------------------*/
char *
Pl_Lexer_Token(StmInf *pstm, Bool comma_is_punct)
{
  int c0;
  char *s;
 ...
 *-------------------------------------------------------------------------*/
static void
Lexer_Number(StmInf *pstm, Bool integer_only)
{
  int lg;
  int radix, radix_c;
  char *p, *f;
  ...
  if (!integer_only &&		/* float if . and digit */
      c == '.' && isdigit(Pl_Lexer_Peek_Char(pstm, TRUE)))
    goto is_a_float;
  ...
  *p = '\0';
  ...
  if (c == '\'')		/* 0'<character> */
    {
      c = Lexer_Quoted_Char(pstm, TRUE, '\'', FALSE);
      if (c == -1)		/* <character> is ' */
	{
  ...
  if (c == 'e' || c == 'E')
    {
      ...
      Read_Next_Char(pstm, TRUE);
      if (!(c_type == DI || ((c == '+' || c == '-') &&
			     isdigit(Pl_Lexer_Peek_Char(pstm, TRUE)))))
	{
	  Unget_Last_Char;
	  c_orig = c_orig0;
	  goto end_float;
...
Lexer_Quoted(StmInf *pstm)
{
  ...
  for (;;)
    {
      c = Lexer_Quoted_Char(pstm, convert, c0, no_escape);
 ...
 *-------------------------------------------------------------------------*/
static int
Lexer_Quoted_Char(StmInf *pstm, Bool convert, int c0, Bool no_escape)
{
  int radix;
  char *p, *f;
  int x, i;
  ...
  if (c == c0)
    {
      if (Pl_Lexer_Peek_Char(pstm, convert) != c0)	/* '' or "" or `` */
 ...
 *-------------------------------------------------------------------------*/
char *
Pl_Lexer_Next_Atom(StmInf *pstm)
{
  char *s;
