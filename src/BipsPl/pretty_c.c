...
static
void      Portray_Clause        (StmInf *pstm,WamWord term_word);

static
Bool      Check_Structure       (WamWord term_word,int func,int arity,
                                 WamWord arg_word[]);
static
Bool      Is_Cut                (WamWord body_word);

static
void      Show_Body             (StmInf *pstm,int level,int context,
                                 WamWord body_word);
static
void      Start_Line            (StmInf *pstm,int level,char c_before);

static
void      Collect_Singleton     (WamWord *adr);

static
int       Var_Name_To_Var_Number(int atom);

static
void      Exclude_A_Var_Number  (int n);

static
void      Collect_Excluded_Rec  (WamWord start_word);

static
void      Bind_Variable         (WamWord *adr,WamWord word);
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
void Portray_Clause_2(WamWord sora_word,WamWord term_word)
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
void Portray_Clause_1(WamWord term_word)
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
static void Show_Body(StmInf *pstm,int level,int context,
                      WamWord body_word)
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
void Name_Singleton_Vars_1(WamWord start_word)
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
Bool Name_Query_Vars_2(WamWord query_list_word,WamWord rest_list_word)
 ...
 for(;;)
    {
     ...
     if (tag==STC && Functor_And_Arity(stc_adr)==equal_2)
        ...
        {
unchanged:
         if (!Get_List(rest_list_word) || 
             !Unify_Value(Car(lst_adr)))
             return FALSE;
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
Bool Bind_Variables_4(WamWord term_word,WamWord exclude_list_word,
