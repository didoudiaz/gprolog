...
leash(full):-      !, leash([call,exit,redo,fail,exception]).
leash(half):-      !, leash([call,redo]).
leash(loose):-     !, leash([call]).
leash(none):-      !, leash([]).
leash(tight):-     !, leash([call,redo,fail,exception]).
...
'$debug_port_mask'(call,     0b00001).

'$debug_port_mask'(exit,     0b00010).

'$debug_port_mask'(redo,     0b00100).

'$debug_port_mask'(fail,     0b01000).

'$debug_port_mask'(exception,0b10000).
...
'$spypoint_condition1'(Goal,Port,Test):-
	...
	('$current_predicate_any'(N/A)
		...
		debug
             ;
                format(debugger_output,
                       'Warning: The predicate ~a/~d is undefined~n',[N,A])),
	fail.                                                            % GC
...
nospyall:-
	...
	write(debugger_output,'All spypoints removed'), nl(debugger_output),
	fail.                                                            % GC
...
'$show_spy_points':-
	write(debugger_output,'Spypoints:'), nl(debugger_output),
	clause('$debug_spy_point'(N,A,_),_),
...
'$debug_list_of_pred1'(N,_,_,_):-
	var(N), !, '$pl_err_instantiation'.

'$debug_list_of_pred1'(_,A1,_,_):-
	var(A1), !, '$pl_err_instantiation'.

'$debug_list_of_pred1'(_,_,A2,_):-
	var(A2), !, '$pl_err_instantiation'.
...
'$debug_list_of_pred1'(N,A1,A2,L):-
	...
	('$current_predicate_any'(N/A),
	 ...
	 fail
         ...
         g_read('$debug_work',L)),
	 (L=[],
          (A1=A2, Z=A1 ; A1=0, A2=Max, Z=any ; Z=A1-A2),
          format(debugger_output,'Warning: spy ~a/~w - no matching predicate~n',
                 [N,Z])
             ;
          true).
...
'$debug_call'(notrace,_):-	!, notrace.
'$debug_call'(nodebug,_):-	!, nodebug.
'$debug_call'(trace,_):-  	!, trace.
'$debug_call'(debug,_):-  	!, debug.
'$debug_call'(debugging,_):-	!, debugging.
'$debug_call'(leash(L),_):-	!, leash(L).
'$debug_call'(spy(Spec),_):-	!, spy(Spec).
'$debug_call'(spypoint_condition(Goal,Port,Test),_):-
                                !, spypoint_condition(Goal,Port,Test).
'$debug_call'(nospy(Spec),_):-	!, nospy(Spec).
'$debug_call'(nospyall,_):-	!, nospyall.


'$debug_call'(Goal,CallInfo):-
	g_read('$debug_info',DebugInfo),
	DebugInfo=d(Invoc,OldAncLst),
	(OldAncLst=[] -> Index=0 ; OldAncLst=[a(_,_,Index,_)|_]),
	Invoc1 is Invoc+1,
	Index1 is Index+1,
	'$get_current_B'(B),
...
%format('starting of call:~w',[Goal]), disp_B(''),
	'$debug_call1'(Goal,CallInfo,Invoc1,Index1,NewAncLst,
                       DebugInfo,Invoc,OldAncLst).
...
%format('Goal:~w  Call Info:~w~n',[Goal,CallInfo]),
        '$get_current_B'(B),
	'$catch_internal'(
          '$debug_call_port'(Goal,CallInfo,Invoc1,Index1,NewAncLst),
          Ball,
          '$debug_exception_port'(Goal,Invoc1,Index1,NewAncLst,Ball),
          0),
        '$get_current_B'(B1),
...
'$debug_call_port'(Goal,CallInfo,Invoc,Index,AncLst):-
	...
	(DebugUnify=='' -> '$call_from_debugger'(Goal,CallInfo)
                        ;  Goal=DebugUnify).
...
'$debug_port_prompt'(Goal,Invoc,Index,AncLst,Port,B):-
	...
	('$has_no_spy_point'(Goal), '$debug_is_not_leashed'(Port)
                ->  nl(debugger_output)
                ;
                   '$debug_read_cmd'(C),
...
'$debug_read_cmd'(C):-
	write(debugger_output,' ? '), flush_output(debugger_output),
...
'$debug_exec_cmd'('s',_,_,_,Port,_):-                                  % skip
	(Port=exit ; Port=fail ; Port=exception),
	!,
	'$debug_exec_cmd'('c',_,_,_,_,_).
...
'$debug_exec_cmd'('G',_,_,_,_,_):-                                     % goto
	...
	g_assign('$debug_next',skip),
 	g_assign('$debug_skip',s(Invoc,0b11111)).
...
'$debug_exec_cmd'('w',Goal,_,_,_,_):-                                 % write
	...
	write(debugger_output,Goal), nl(debugger_output),
	!,
	fail.

'$debug_exec_cmd'('d',Goal,_,_,_,_):-                               % display
	write(debugger_output,'                    '),
	display(debugger_output,Goal), nl(debugger_output),
	!,
	fail.

'$debug_exec_cmd'('p',Goal,_,_,_,_):-                                 % print
	write(debugger_output,'                    '),
	print(debugger_output,Goal), nl(debugger_output),
	!,
	fail.
...
'$debug_exec_cmd'('.',Goal,_,_,_,_):-                           % father file
	...
	('$get_predicate_file_info'(N/A,PlFile,PlLine)
             -> format(debugger_output,'~a/~d defined in ~a:~d~n',
                       [N,A,PlFile,PlLine])
...
'$debug_exec_cmd'('@',_,_,_,_,_):-                                  % command
	...
	('$catch'(Command,
                  Err,
                  format(debugger_output,'Warning: ~w - exception raised ~w~n',[Command,Err]),
...
'$debug_write_goal'(Goal,Invoc,Index,Port):-
	...
	('$has_no_spy_point'(Goal) -> Indic=' ' ; Indic=(+)),
...
'$debug_disp_anc_lst'([a(_,_,_,_)|AncLst]):-
	write(debugger_output,'Ancestors:'), nl(debugger_output),
...
'$debug_disp_alternatives'(AncLst,B):-
	write(debugger_output,'Alternatives:'), nl(debugger_output),
...
'$debug_disp_alt1'('$trail_handler',1,_).                  % hide debug catch

'$debug_disp_alt1'(N,A,_):-                         % detect system predicate
	sub_atom(N,0,1,_,'$'),
	'$predicate_property_any'(N/A,native_code),
	((sub_atom(N,1,_,4,N1), '$debug_check_bip'(N1,A1) 
                   ;
          sub_atom(N,1,_,1,N1), '$debug_check_bip'(N1,A1))
             -> '$debug_disp_alt2'(N1/A1)
