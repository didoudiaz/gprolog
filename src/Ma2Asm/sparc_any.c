...

{
 ...
 if (LITTLE_INT(int_val))
     Delay_Printf("mov","%d,%s",int_val,r);
 else
    {
     Delay_Printf("sethi","%%hi(%d),%s",int_val,r);
     Delay_Printf("or","%s,%%lo(%d),%s",r,int_val,r);
    }
...
{
 if (LITTLE_INT(int_val))
     Inst_Printf("cmp","%%o0,%d",int_val);
 else
    {
     Inst_Printf("sethi","%%hi(%d),%%o1",int_val);
     Inst_Printf("or","%%o1,%%lo(%d),%%o1",int_val);
...
#ifdef M_sparc_sunos
         if (!global)
             Inst_Printf(".reserve",UN "%s,%d,\"bss\",4",name,value*4);
          else
             Inst_Printf(".common",UN "%s,%d,\"bss\"",name,value*4);
#else
         if (!global)
             Inst_Printf(".local",UN "%s",name);
         Inst_Printf(".common",UN "%s,%d,4",name,value*4);
#endif
         break;
