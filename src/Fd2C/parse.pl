...
parse_user_cstr(uc(Name,AFSize,LHVar,Body)) --> 
	...
	{ g_assign(afsize,NbHVar),
          retractall(hvar(_)), asserta(hvar(LHVar)),
...
type(int)     --> terminal(int).
type(range)   --> terminal(range).
type(fdv)     --> terminal(fdv).
type(any)     --> terminal(any).
type(l_int)   --> terminal(l_int).
type(l_range) --> terminal(l_range).
type(l_fdv)   --> terminal(l_fdv).
type(l_any)   --> terminal(l_any).  
...
bloc_one(bl(BNo,LDep,LUse,LWInst,TellFdv,Always)) -->
	...
	{ (HasForAll=t -> LWInst3=[fd_forall_end]
                       ;  LWInst3=[]) },
	trig(LVar,LUse,LDep),
	always(Always),
...
trig(LVar,LUse,LDep) -->
	...
	!,
	terminal_check(on),
	trig_lst(LVar,LDep), { add_use_to_dep(LUse,LDep) }.
...
trig_one(LVar,LDep) -->
	terminal(What), { What=min ; What=max ; What=dom ; What=val },
	!,
	terminal_check('('), 
	a_var(V), 
	terminal_check(')'),
	{ get_typeof(LVar,V,T,I),
	  (T\==fdv, T\==l_fdv 
               -> var_check_type(LVar,V,fdv,_,_,_)               % type error
               ;  true),
...
last_elem(LVar,LUse,-1,LWNext,LWInst) -->
	c_fct(LVar,LUse,Head,LArg,LWInst1,LWInst),
 	{ Term=..[Head|LArg],
	  LWInst1=[fd_check_fct(Term)|LWNext] }.
...
x_in_r(LVar,LUse,I,LWNext,LWInst) -->
	var_check_type(LVar,_,fdv,I), terminal_check(in), 
...
r_prim(LVar,LUse,Range,LWNext,LWInst) -->
	terminal(dom),
	terminal_check('('), var_check_type(LVar,V,fdv,I), 
	terminal_check(')'), !,
...
t_mul(LVar,LUse,Term,LWNext,LWNext) -->
	terminal(What), { What=min ; What=max ; What=val },
	terminal_check('('), 
...
t_mul(LVar,LUse,Term,LWNext,LWNext) -->
	a_var(V),
	{ get_typeof(LVar,V,int,I), !,
	  atom_concat(int,V,Term),
	  add_marked_var(LUse,V,int,I,_) }.
...
syn_error(Expected) -->
	[t(T,L,C)],
	{ error('~d: syntax error : ~w expected at "~w" (char:~d)',
          [L,Expected,T,C]) }.


sem_error(Msg,Args) -->
	[t(_,L,C)],
	{ append([L,Msg|Args],[C],M),
          error('~d: ~? (char:~d)',M) }.
...
add_marked_var([m(V,T,I,Mark)|_],V,T,I,What):-
	!,
	((T=fdv ; T=l_fdv), nonvar(What)
            -> (What=min,    Mark=i(t,_,_,_)
                        ;
                What=max,    Mark=i(_,t,_,_)
                        ;
                What=dom(R), Mark=i(_,_,t(R),_)
                        ;
                What=val,    Mark=i(_,_,_,t)), !
            ;
	        Mark=What).
