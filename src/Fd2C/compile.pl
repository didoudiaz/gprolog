...
emit_user_cstr(uc(Name,AFSize,LHVar,Body)):-
	...
	atom_length(Name,X),
	X1 is X+19,
	format(stream_c,'~n\t/*~*c*/~n',[X1,0'-]),
	format(stream_c,'\t/* User constraint: ~a */~n',[Name]),
	format(stream_c,'\t/*~*c*/~n',[X1,0'-]),
...
e_test_case_lst([ca(co(Term,LWNext,LWInst),_,_)|LCase],[FctName|LFctName],
                LWInst):-
...
e_bloc_load_fdv(val,Min,Max,Dom,_,V,I,LCVarUsed,LWNext,LWInst):-
	...
	(nonvar(Dom) -> Dom=t(R),
	                LWInst1=[fd_range_interval(R,AVal,AVal)|LWInst2]
                     ;  LWInst1=LWInst2),
	(nonvar(Min) -> use_c_var(min,V,LCVarUsed,AMin),
                        LWInst2=[fd_value_copy(AMin,AVal)|LWInst3]
                     ;  LWInst2=LWInst3),
	(nonvar(Max) -> use_c_var(max,V,LCVarUsed,AMax),
...
e_compute_dep_chain(Min,Max,_,_,min):-
	nonvar(Min), var(Max),                            /* only min used */
        !.

e_compute_dep_chain(Min,Max,_,_,max):-
	var(Min), nonvar(Max),                            /* only max used */
        !.
...
simplif_code([fd_range_interval(R1,T1,T2),fd_tell_range(I,R2)|LWInst],
             LWSimpl):-
	R1==R2,
	!,
...
simplif_code([fd_range_full(R1),fd_range_reset_value(R2,T),
             fd_tell_range(I,R3)|LWInst],LWSimpl):-
	R1==R2,
	R1==R3,
	!,
...
simplif_code([fd_range_interval(R1,T1,T2),fd_range_union(R2,R3)|LWInst],
             LWSimpl1):-
...
alloc_and_emit_fct_code(FctName,LCVarUsed,LWInst,LocalFdvAdr):-
	...
	(LocalFdvAdr=t -> format(stream_c,'   fd_local_fdv_adr~n',[])
                       ;  true),
	emit_c_vars_lst(LCVarUsed),
	emit_used_regs_lst(MaxUsedReg),
	nl(stream_c),
	(MaxUsedReg>=0 -> format(stream_c,'   fd_allocate~n',[]),
                          Alloc=t
                       ;  Alloc=f),
...
emit_inst_lst([],Alloc):-
	format(stream_c,' fd_exit_point~n',[]),
	(Alloc=t -> format(stream_c,'   fd_deallocate~n',[])
                 ;  true),
	format(stream_c,'   fd_return~n',[]).
