...
DLLEXPORT void W32GC_Backd(int fd_out, int n);
...
MainWndProc_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
  switch (id)
    ...
    case IDM_BUFFERING:
      ...
      if (!line_buffering)
	{
	  CheckMenuItem(GetMenu(hwnd), IDM_BUFFERING,
			MF_BYCOMMAND | MF_UNCHECKED);
	  EnableMenuItem(GetMenu(hwnd), IDM_FLUSH,
...
W32GC_Confirm_Box(char *title, char *msg)
{
  ...
  Flush_Buffer();		/* synchronize output and posit */
  return (MessageBox(hwndMain, msg, title,
...
DLLEXPORT void
W32GC_Message_Box(char *title, char *msg)
{
  Flush_Buffer();		/* synchronize output and posit */
