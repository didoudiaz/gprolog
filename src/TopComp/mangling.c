...
/* This file is used by top_comp.c, mangle_filter.c and wam2ma.c */
 ...
 *-------------------------------------------------------------------------*/
char *
Mangle(char *module, char *pred, int arity, char *str)
{
 ...
 * strict    : TRUE: only predicate indicators, FLSE: also predicate names *
 *                                                                         *
 * Returns an encoded line (static buffer)                                 *
 *-------------------------------------------------------------------------*/
char *
Mangle_Line(char *str, char *format, Bool strict)
{
  ...
  for(;;)
	...
	*dst++ = *src++;
	...
	src = q;
      ...
      Mangle(module, pred, arity, free_in_buff);
 ...
 * strict    : TRUE: only predicate indicators, FALSE: also predicate names*
 * quote     : TRUE: surround module|pred with quotes if needed            *
 ...
 *-------------------------------------------------------------------------*/
char *
Demangle(char *str, Bool strict, Bool quote, int decode_aux,
 ...
 *-------------------------------------------------------------------------*/
char *
Demangle_Line(char *str, char *format, Bool strict, Bool quote, int decode_aux)
{
  static char result[4096];
  ...
  while(*src)
    {
	  ...
	  ((p = Demangle(p, strict, quote, decode_aux, module, pred, &arity, &aux_no)) != NULL))
	{
	  src = p;
