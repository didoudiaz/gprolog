 ...
 * ENCODE_HEXA_CX                                                          *
 *                                                                         *
 * unit      : unit to encode (or NULL or '\0' if none) - overrides module *
 * uarity    : unit arity to encode - zero if unit NULL or \0              *
 ...
 *-------------------------------------------------------------------------*/
char *
Encode_Hexa_CX(char *unit, int uarity,
	       char *module, char *pred, int arity, char *str)
{
  ...
  if (module && unit) {	      /* explicit module => context ignored */
    ...
    module = unit;		/* unit becomes module */
 ...
 * ENCODE_HEXA                                                             *
 *                                                                         *
 * Like ENCODE_HEX_CX but defaults no NULL unit                            *
 *-------------------------------------------------------------------------*/
char *
Encode_Hexa(char *module, char *pred, int arity, char *str)
{
  return Encode_Hexa_CX (NULL, 0, module, pred, arity, str);
}
 ...
 * unit      : the resulting decoded unit (or '\0' if none)                *
 * uarity    : the resulting unit arity                                    *
	    ...
	    char *module, char *pred, int *arity, int *aux_no)
{
  ...
  if (module_encode == 0) { 	/* no module qualif */
    ...
    if (*str == 'u') {		/* unit arity coming up */
      ++str;
      *uarity = strtoul (str, &p, 10); /* read the number */
      if (p == str)
	*uarity = -1;		/* badly formed! */
      str = p;
      ++str;			/* skip over one '_' */
      strcpy (unit, module);	/* it's actually a unit name */
      *module = '\0';		/* forget about the module */
    }
...
Decode_Hexa_Line(char *str, char *format, int strict, int quote, int decode_aux)
{
  ...
  static char unit[2048];	/* a bit too big? */
  ...
  while(*src)
    {
	  ...
	  ((p = Decode_Hexa(p, strict, quote, decode_aux, 
			    unit, &uarity,
			    module, pred, &arity, &aux_no)) != NULL))
	{
	  src = p;
