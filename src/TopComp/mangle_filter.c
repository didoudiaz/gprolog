 ...
 *                                                                         *
 * Part  : Utiliy                                                          *
 * File  : mangle_filter.c                                                 *
 * Descr.: Prolog (de)mangling filter                                      *
 * Author: Daniel Diaz                                                     *
 *                                                                         *
...
#define GPLC_FILTER_VERSION        "1.2"
...
One_Line(char *str)
{
  ...
  if (encode)
    fputs(Mangle_Line(str, format, strict), stdout);
  else
    fputs(Demangle_Line(str, format, strict, quote, decode_aux), stdout);
}
  ...
  for (i = 1; i < argc; i++)
      ...
      if (*argv[i] == '-' && argv[i][1] != '\0')
	{
	  ...
	  if (Check_Arg(i, "--version"))
	    {
	      fprintf(stderr, "GNU Prolog demangling Filter");
	      fprintf(stderr, "%s version %s\n", GPLC_FILTER, GPLC_FILTER_VERSION);
	      exit(0);
	    }
...
{
  fprintf(stderr, "Usage: %s [OPTION]... [FILE...]", GPLC_FILTER);
  L(" ");
  L("Options:");
