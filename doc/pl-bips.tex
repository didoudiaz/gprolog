...
\IdxPBD[var/1]{var(Term)} succeeds if \texttt{Term} is currently
uninstantiated (which therefore has not been bound to anything, except
possibly another uninstantiated variable).

\IdxPBD[nonvar/1]{nonvar(Term)} succeeds if \texttt{Term} is currently instantiated
(opposite of \texttt{var/1}).

\IdxPBD[atom/1]{atom(Term)} succeeds if \texttt{Term} is currently instantiated
to an atom.

\IdxPBD[integer/1]{integer(Term)} succeeds if \texttt{Term} is currently
instantiated to an integer.

\IdxPBD[float/1]{float(Term)} succeeds if \texttt{Term} is currently instantiated
to a floating point number.

\IdxPBD[number/1]{number(Term)} succeeds if \texttt{Term} is currently instantiated
to an integer or a floating point number.

\IdxPBD[atomic/1]{atomic(Term)} succeeds if \texttt{Term} is currently instantiated
to an atom, an integer or a floating point number.

\IdxPBD[compound/1]{compound(Term)} succeeds if \texttt{Term} is currently
instantiated to a compound term, i.e. a term of arity $>$ 0 (a list or a
structure).

\IdxPBD[callable/1]{callable(Term)} succeeds if \texttt{Term} is currently
instantiated to a callable term, i.e. an atom or a compound term.

\IdxPBD[list/1]{list(Term)} succeeds if \texttt{Term} is currently instantiated
to a list, i.e. the atom \texttt{[]} (empty list) or a term with principal
functor \texttt{'.'/2} and with second argument (the tail) a list.

\IdxPBD[partial\_list/1]{partial\_list(Term)} succeeds if \texttt{Term} is currently
...
\IdxPBD[list\_or\_partial\_list/1]{list\_or\_partial\_list(Term)} succeeds if \texttt{Term} is
...
\IdxPBD[(=)/2]{Term1 = Term2} unifies \texttt{Term1} and \texttt{Term2}. No occurs
...
\IdxPBD[unify\_with\_occurs\_check/2]{unify\_with\_occurs\_check(Term1, Term2)} unifies \texttt{Term1} and
...
\IdxPBD[({\bs}=)/2]{Term1 {\bs}= Term2} succeeds if \texttt{Term1} and \texttt{Term2}
...
\IdxPBD[(==)/2]{Term1 == Term2} succeeds if \texttt{Term1} and \texttt{Term2} are
equal. 

\IdxPBD[({\bs}==)/2]{Term1 {\bs}== Term2} succeeds if \texttt{Term1} and
\texttt{Term2} are different.

\IdxPBD[("@{\lt})/2]{Term1 @{\lt} Term2} succeeds if \texttt{Term1} is less than
\texttt{Term2}.

\IdxPBD[("@={\lt})/2]{Term1 @={\lt} Term2} succeeds if \texttt{Term1} is less than or
equal to \texttt{Term2}.

\IdxPBD[("@{\gt})/2]{Term1 @{\gt} Term2} succeeds if \texttt{Term1} is greater than
\texttt{Term2}.
...
\IdxPBD[compare/3]{compare(Result, Term1, Term2)} compares \texttt{Term1} and
\texttt{Term2} according to the standard \RefSP{Standard-total-ordering-of-terms} and unifies \texttt{Result} with:
...
\ErrCond{\texttt{Result} is neither a variable nor an atom}
\ErrTerm{type\_error(atom, Result)}
...
\IdxPBD[functor/3]{functor(Term, Name, Arity)} succeeds if the principal functor of
...
\IdxPBD[arg/3]{arg(N, Term, Arg)} succeeds if the \texttt{N}\emph{th} argument of
...
\IdxPBD[(=..)/2]{Term =.. List} succeeds if \texttt{List} is a list whose head is the
...
\subsubsection{\texttt{copy\_term/2}}
...
\IdxPBD[copy\_term/2]{copy\_term(Term1, Term2)} succeeds if \texttt{Term2} unifies with a
...
\IdxPBD[setarg/4]{setarg(N, Term, NewValue, Undo)} replaces destructively the
...
\IdxPBD[setarg/3]{setarg(N, Term, NewValue)} is equivalent to \texttt{setarg(N, Term,
NewValue, true)}.
...
\IdxPBD[name\_singleton\_vars/1]{name\_singleton\_vars(Term)} binds each singleton variable appearing
...
\IdxPBD[name\_query\_vars/2]{name\_query\_vars(List, Rest)} for each element of \texttt{List} of
...
\texttt{read\_term/3} with \IdxPO[variable\_names]{variable\_names(List)} or
\IdxPO[singletons]{singletons(List)} options \RefSP{read-term/3}.
...
\IdxPBD[bind\_variables/2]{bind\_variables(Term, Options)} binds each variable appearing in
...
\item \IdxPOD[from]{from(From)}: the first integer \texttt{N} to use for
number/name variables of \texttt{Term} is \texttt{From}. The default value
is \texttt{0}.

\item \IdxPOD[next]{next(Next)}: when \texttt{bind\_variables/2} succeeds,
\texttt{Next} is unified with the (last integer \texttt{N})+1 used
to bind the variables of \texttt{Term}. 

\item \IdxPOD[exclude]{exclude(List)}: collects all variable names appearing
...
(with \IdxPO[variable\_names]{variable\_names(List)} or
\IdxPO[singletons]{singletons(List)} options) \RefSP{read-term/3} and by
...
\IdxPBD[numbervars/3]{numbervars(Term, From, Next)} is equivalent to
...
\IdxPBD[numbervars/1]{numbervars(Term)} is equivalent to \texttt{numbervars(Term, 0, \_)}.
...
\subsubsection{\texttt{term\_ref/2}}
...
\IdxPBD[term\_ref/2]{term\_ref(Term, Ref}) succeeds if the internal reference of
...
\begin{tabular}{|l|L{7.4cm}|c|c|}
\hline

Expression & Result = \textit{eval}(Expression) & Signature & ISO \\

\hline\hline

Variable & must be bound to a non-variable expression \texttt{E}. 
%%%%%%%% line break: a blank line works for both HEVEA and LaTeX %%%%%%%

           The result is \textit{eval}(\texttt{E})
         & IF $\rightarrow$ IF & Y \\
...
\texttt{acos(E)} & arc cosine of \textit{eval}(\texttt{E}) & IF $\rightarrow$ F &
N \\
...
\texttt{asin(E)} & arc sine of \textit{eval}(\texttt{E}) & IF $\rightarrow$ F & N \\
...
\texttt{log(E)} & natural logarithms of \textit{eval}(\texttt{E}) & IF
$\rightarrow$ F & Y \\
...
and \texttt{-} are predefined prefix operators \RefSP{op/3:(Term-input/output)}.
...
the \IdxPF{integer\_rounding\_function} \Idx{Prolog flag} to determine which
function being used \RefSP{set-prolog-flag/2}.
...
\subsubsection{\texttt{(is)/2} - evaluate expression}
...
\IdxPBD[(is)/2]{Result is Expression} succeeds if \texttt{Result} can be
...
\IdxPBD[(=:=)/2]{Expr1 =:= Expr2} succeeds if \textit{eval}(\texttt{Expr1}) =
\textit{eval}(\texttt{Expr2}).

\IdxPBD[(={\bs}=)/2]{Expr1 ={\bs}= Expr2} succeeds if \textit{eval}(\texttt{Expr1})
$\neq$ \textit{eval}(\texttt{Expr2}).

\IdxPBD[({\lt})/2]{Expr1 {\lt} Expr2} succeeds if \textit{eval}(\texttt{Expr1}) $<$
\textit{eval}(\texttt{Expr2}).

\IdxPBD[(={\lt})/2]{Expr1 ={\lt} Expr2} succeeds if \textit{eval}(\texttt{Expr1})
$\leq$ \textit{eval}(\texttt{Expr2}).

\IdxPBD[({\gt})/2]{Expr1 {\gt} Expr2} succeeds if \textit{eval}(\texttt{Expr1}) $>$
\textit{eval}(\texttt{Expr2}).
...
\IdxPBD[asserta/1]{asserta(Clause)} first converts the term \texttt{Clause} to a clause
...
\IdxPBD[assertz/1]{assertz(Clause)} acts like \texttt{asserta/1} except that the clause
...
\IdxPBD[retract/1]{retract(Clause)} erases the first clause of the database
...
\IdxPBD[retractall/1]{retractall(Head)} erases all clauses whose head unifies with
...
\IdxPBD[clause/2]{clause(Head, Body)} succeeds if there exists a clause in the
...
\IdxPBD[abolish/1]{abolish(Pred)} removes from the database the procedure whose
...
\subsubsection{\texttt{current\_predicate/1}}
\label{current-predicate/1}
...
\IdxPBD[current\_predicate/1]{current\_predicate(Pred)} succeeds if there
...
\IdxPBD[predicate\_property/2]{predicate\_property(Pred, Property)} succeeds if
\texttt{current\_predicate(Pred)} succeeds \RefSP{current-predicate/1} and
if \texttt{Property} unifies with one of the properties of the procedure.
...
\item \IdxPPD{built\_in}: if the procedure is a Prolog built-in predicate.
...
\item \IdxPPD[prolog\_file]{prolog\_file(File)}: source file from which the
predicate has been read.

\item \IdxPPD[prolog\_line]{prolog\_line(Line)}: line number of the source
file.
...
\IdxPBD[findall/3]{findall(Template, Goal, Instances)} succeeds if \texttt{Instances}
...
\IdxPBD[bagof/3]{bagof(Template, Goal, Instances)} assembles as a list the
...
the form \texttt{T\^{}Goal} and exclude all variables occuring in \texttt{T}
...
\IdxPBD[setof/3]{setof(Template, Goal, Instances)} is equivalent to
...
associated to these pipes, e.g. \IdxPB{popen/3} \RefSP{popen/3}.
...
automatically ends when the stream is closed. A particular alias only refers
to at most one stream at any one time. However, more than one alias can be
associated to a stream. All built-in predicates which have a stream-term as
an input argument also accept a stream alias as that argument. However,
built-in predicates which return a stream-term do not accept a stream alias.

\SPart{Standard streams}: two streams are predefined and open during the
execution of every goal: the standard input stream which has the alias
\IdxPKD{user\_input} and the standard output stream which has the alias
\IdxPKD{user\_output}. A goal which attempts to close either standard stream
...
use to reposition a stream (when possible) using for instance
...
\IdxPBD[current\_input/1]{current\_input(Stream)} unifies \texttt{Stream} with the stream-term
...
\IdxPBD[current\_output/1]{current\_output(Stream)} unifies \texttt{Stream} with the
...
\IdxPBD[set\_input/1]{set\_input(SorA)} sets the current input stream to be the stream
...
\subsubsection{\texttt{set\_output/1}}
\label{set-output/1}
...
\IdxPBD[set\_output/1]{set\_output(SorA)} sets the current output stream to be the stream
associated with the stream-term or alias \texttt{SorA}.
...
\IdxPBD[open/4]{open(SourceSink, Mode, Stream, Options)} opens the source/sink
...
\item \IdxPOD[type]{type(text}/\texttt{binary)}: specifies whether the
stream is a text stream or a binary stream. The default value is
\texttt{text}.

\item \IdxPOD[reposition]{reposition(true}/\texttt{false)}: specifies
...
\item \IdxPOD[eof\_action]
{eof\_action(error}/\texttt{eof\_code}/\texttt{reset)}:
...
\item \IdxPOD[alias]{alias(Alias)}: specifies that the atom \texttt{Alias}
is to be an alias for the stream. By default no alias is attached to the
stream.

\item \IdxPOD[buffering]{buffering(none}/\texttt{line}/\texttt{block)}:
...
\IdxPBD[open/3]{open(SourceSink, Mode, Stream, Options)} is equivalent to
...
\ErrTerm{type\_error(variable, Stream)}
...
ISO predicates. The \texttt{buffering/1} stream option is a GNU Prolog
extension.
...
\IdxPBD[close/2]{close(SorA, Options)} closes the stream associated with the stream-term
...
\item \IdxPOD[force]{force(true}/\texttt{false)}: with \texttt{false}, if
...
\IdxPBD[close/1]{close(SorA)} is equivalent to \texttt{close(SorA, [])}.
...
\subsubsection{\texttt{flush\_output/1},
\texttt{flush\_output/0}}
\label{flush-output/1}
...
\IdxPBD[flush\_output/1]{flush\_output(SorA)} sends any buffered output characters/bytes to
the stream.

\IdxPBD[flush\_output/0]{flush\_output/0} applies to the current output stream.
...
\IdxPBD[current\_stream/1]{current\_stream(Stream)} succeeds if there exists
...
\IdxPBD[stream\_property/2]{stream\_property(Stream, Property)} succeeds if
...
\item \IdxPPD[file\_name]{file\_name(F)}: the name of the connected 
source/sink.

\item \IdxPPD[mode]{mode(M)}: \texttt{M} is the open mode (\texttt{read},
...
\item \IdxPPD[alias]{alias(A)}: \texttt{A} is an alias of the stream.

\item \IdxPPD[type]{type(T)}: \texttt{T} is the type of the stream
(\texttt{text}, \texttt{binary}).

\item \IdxPPD[reposition]{reposition(R)}: \texttt{R} is the reposition
boolean (\texttt{true}, \texttt{false}).

\item \IdxPPD[eof\_action]{eof\_action(A)}: \texttt{A} is the end-of-file
action (\texttt{error}, \texttt{eof\_code}, \texttt{reset}).

\item \IdxPPD[buffering]{buffering(B)}: \texttt{B} is the buffering mode
(\texttt{none}, \texttt{line}, \texttt{block}).

\item \IdxPPD[end\_of\_stream]{end\_of\_stream(E)}: \texttt{E} is the
...
\item \IdxPPD[position]{position(P)}: \texttt{P} is the stream-position
term associated to the current position.
...
\subsubsection{\texttt{at\_end\_of\_stream/1},
\texttt{at\_end\_of\_stream/0}}
...
\IdxPBD[at\_end\_of\_stream/1]{at\_end\_of\_stream(SorA)} succeeds if the stream associated with
...
\IdxPBD[at\_end\_of\_stream/0]{at\_end\_of\_stream/0} applies to the current input stream.
...
\IdxPBD[stream\_position/2]{stream\_position(SorA, Position)} succeeds unifying
\texttt{Position} with the stream-position term associated to the current
...
\IdxPBD[set\_stream\_position/2]{set\_stream\_position(SorA, Position)} sets the position of
...
\IdxPBD[seek/4]{seek(SorA, Whence, Offset, NewOffset)} sets the position of
the stream associated with the stream-term or alias \texttt{SorA} to
...
\subsubsection{\texttt{character\_count/2}}
...
\IdxPBD[character\_count/2]{character\_count(SorA, Count)} unifies \texttt{Count} with the
...
\subsubsection{\texttt{line\_count/2}}
\label{line-count/2}
...
\IdxPBD[line\_count/2]{line\_count(SorA, Count)} unifies \texttt{Count} with the number of
...
\IdxPBD[line\_position/2]{line\_position(SorA, Count)} unifies
...
\IdxPBD[stream\_line\_column/3]{stream\_line\_column(SorA, Line, Column)} unifies \texttt{Line}
...
\IdxPBD[set\_stream\_line\_column/3]{set\_stream\_line\_column(SorA, Line, Column)} sets the stream
...
\subsubsection{\texttt{add\_stream\_alias/2}}
...
\IdxPBD[add\_stream\_alias/2]{add\_stream\_alias(SorA, Alias)} adds \texttt{Alias} as a new alias
...
\IdxPBD[current\_alias/2]{current\_alias(Stream, Alias)} succeeds if
\texttt{current\_stream(Stream)} succeeds \RefSP{current-stream/1} and if
...
\IdxPBD[set\_stream\_type/2]{set\_stream\_type(SorA, Type)} updates the type associated with
...
\subsubsection{\texttt{set\_stream\_eof\_action/2}}
...
\IdxPBD[set\_stream\_eof\_action/2]{set\_stream\_eof\_action(SorA, Action)}
...
\IdxPBD[set\_stream\_buffering/2]{set\_stream\_buffering(SorA, Buffering)}
...
\IdxPBD[open\_input\_atom\_stream/2]{open\_input\_atom\_stream(Atom, Stream)} unifies \texttt{Stream}
with the stream-term which is associated to a new input text-stream whose
data are the characters of \texttt{Atom}.

\IdxPBD[open\_input\_chars\_stream/2]{open\_input\_chars\_stream(Chars, Stream)} is similar to
\texttt{open\_input\_atom\_stream/2} except that data are the content of the
character list \texttt{Chars}.

\IdxPBD[open\_input\_codes\_stream/2]{open\_input\_codes\_stream(Codes, Stream)} is similar to
\texttt{open\_input\_atom\_stream/2} except that data are the content of the
...
\subsubsection{\texttt{close\_input\_atom\_stream/1},
\texttt{close\_input\_chars\_stream/1}, \\ 
\texttt{close\_input\_codes\_stream/1}}
...
\IdxPBD[close\_input\_atom\_stream/1]{close\_input\_atom\_stream(SorA)} closes the constant term stream
...
\IdxPBD[close\_input\_chars\_stream/1]{close\_input\_chars\_stream(SorA)} acts similarly for a character
list stream.

\IdxPBD[close\_input\_codes\_stream/1]{close\_input\_codes\_stream(SorA)} acts similarly for a character
code list stream.
...
\IdxPBD[open\_output\_atom\_stream/1]{open\_output\_atom\_stream(Stream)} unifies \texttt{Stream} with the
...
the stream is closed by \texttt{close\_ouput\_atom\_stream/2}
\RefSP{close-output-atom-stream/2}.

\IdxPBD[open\_output\_chars\_stream/1]{open\_output\_chars\_stream(Stream)} is similar to
\texttt{open\_output\_atom\_stream/1} except that the result will be a
character list.

\IdxPBD[open\_output\_codes\_stream/1]{open\_output\_codes\_stream(Stream)} is similar to
\texttt{open\_output\_atom\_stream/1} except that the result will be a
character code list.
...
\IdxPBD[close\_output\_atom\_stream/2]{close\_output\_atom\_stream(SorA, Atom)} closes the constant term
stream associated with the stream-term or alias \texttt{SorA}. \texttt{SorA} must
be associated to a stream open with \texttt{open\_output\_atom\_stream/1}
\RefSP{open-output-atom-stream/1}.  \texttt{Atom} is unified with an atom
formed with all characters written on the stream.

\IdxPBD[close\_output\_chars\_stream/2]{close\_output\_chars\_stream(SorA, Chars)} acts similarly for a
character list stream.

\IdxPBD[close\_output\_codes\_stream/2]{close\_output\_codes\_stream(SorA, Codes)} acts similarly for a
...
\subsubsection{\texttt{get\_char/2},
\texttt{get\_char/1}, 
\texttt{get\_code/1},
\texttt{get\_code/2}}
\label{get-char/2}
...
\IdxPBD[get\_char/2]{get\_char(SorA, Char)} succeeds if \texttt{Char} unifies with the
next character read from the stream associated with the stream-term or alias
\texttt{SorA}.

\IdxPBD{get\_code/2} is similar to \texttt{get\_char/2} but deals with
character codes.

\IdxPBD{get\_char/1} and \IdxPBD{get\_code/1} apply to the current input
stream.
...
\subsubsection{\texttt{get\_code\_no\_echo/2},
\texttt{get\_code\_no\_echo/1}}

\begin{TemplatesOneCol}
get\_code\_no\_echo(+stream\_or\_alias, ?integer)\\
get\_code\_no\_echo(?integer)

\end{TemplatesOneCol}

\Description

\IdxPBD[get\_code\_no\_echo/2]{get\_code\_no\_echo/2} is similar to \IdxPB{get\_code/2}
\RefSP{get-char/2} except that if the associated source is a terminal the
input is not echoed. This is possible only if the \IdxK{linedit} facility
has been installed \RefSP{The-line-editor} otherwise
\IdxPBD[get\_code\_no\_echo/1]{get\_code\_no\_echo/2} is similar to
\texttt{get\_code/2}. Any integer can be returned (not only an in-character
code) to allow for function key reading.

\texttt{get\_code\_no\_echo/1} applies to the current input stream.
...
\subsubsection{\texttt{peek\_char/2},
\texttt{peek\_char/1},
\texttt{peek\_code/1},
\texttt{peek\_code/2}}
...
\IdxPBD[peek\_char/2]{peek\_char(SorA, Char)} succeeds if \texttt{Char} unifies with the
...
\IdxPBD{peek\_code/2} is similar to \texttt{peek\_char/2} but deals with
character codes.

\IdxPBD{peek\_char/1} and \IdxPBD{peek\_code/1} apply to the current input
stream.
...
\subsubsection{\texttt{unget\_char/2},
\texttt{unget\_char/1},
\texttt{unget\_code/2},
\texttt{unget\_code/1}}
...
\IdxPBD[unget\_char/2]{unget\_char(SorA, Char)} pushes back \texttt{Char}
...
\IdxPBD{unget\_code/2} is similar to \texttt{unget\_char/2} but deals with
character codes.

\IdxPBD{unget\_char/1} and \IdxPBD{unget\_code/1} apply to the current input
stream.
...
\subsubsection{\texttt{put\_char/2},
\texttt{put\_char/1},
\texttt{put\_code/1},
\texttt{put\_code/2},
\texttt{nl/1},
...
\IdxPBD[put\_char/2]{put\_char(SorA, Char)} writes \texttt{Char} onto the
stream associated with the stream-term or alias \texttt{SorA}.

\IdxPBD{put\_code/2} is similar to \texttt{put\_char/2} but deals with
character codes.

\IdxPBD[nl/1]{nl(SorA)} writes a new-line character onto the stream
associated with the stream-term or alias \texttt{SorA}. This is equivalent to
\texttt{put\_char(SorA, '{\bs}n')}.

\IdxPBD{put\_char/1}, \IdxPBD{put\_code/1} and \IdxPBD{nl/0} apply to the
...
\subsubsection{\texttt{get\_byte/2},
\texttt{get\_byte/1}}
...
\IdxPBD[get\_byte/2]{get\_byte(SorA, Byte)} succeeds if \texttt{Byte} unifies with the
next byte read from the stream associated with the stream-term or alias
\texttt{SorA}.

\IdxPBD[get\_byte/1]{get\_byte/1} applies to the current input stream.
...
\subsubsection{\texttt{peek\_byte/2},
\texttt{peek\_byte/1}}
...
\IdxPBD[peek\_byte/2]{peek\_byte(SorA, Byte)} succeeds if \texttt{Byte} unifies with the
next byte that will be read from the stream associated with the stream-term or
alias \texttt{SorA}. The byte is not read.

\IdxPBD[peek\_byte/1]{peek\_byte/1} applies to the current input stream.
...
\subsubsection{\texttt{unget\_byte/2},
\texttt{unget\_byte/1}}
...
\IdxPBD[unget\_byte/2]{unget\_byte(SorA, Byte)} pushes back \texttt{Byte} onto the stream
...
\IdxPBD[unget\_byte/1]{unget\_byte/1} applies to the current input stream.
...
\IdxPBD[put\_byte/2]{put\_byte(SorA, Byte)} writes \texttt{Byte} onto the stream
associated with the stream-term or alias \texttt{SorA}. 

\IdxPBD[put\_byte/1]{put\_byte/1} applies to the current output stream.
...
\IdxPF{double\_quotes} Prolog flag \RefSP{set-prolog-flag/2}.
...
\IdxPBD[read\_term/3]{read\_term(SorA, Term, Options)} is true if
...
\item \IdxPOD[variables]{variables(VL)}: \texttt{VL} is unified with the
...
\item \IdxPOD[variable\_names]{variable\_names(VNL)}: \texttt{VNL} is
unified with the list of pairs \texttt{Name = Var} where \texttt{Var} is a
named variable of the term and \texttt{Name} is the atom associated to the
name of \texttt{Var}.  Anonymous variables are not included in the list
\texttt{VNL}.

\item \IdxPOD[singletons]{singletons(SL)}: \texttt{SL} is unified with the
...
\item \IdxPOD[syntax\_error]
{syntax\_error(error}/\texttt{warning}/\texttt{fail)}: specifies the effect
of a syntax error:
...
\item \IdxPOD[end\_of\_term]{end\_of\_term(dot}/\texttt{eof)}: specifies the
...
\IdxPB{read\_from\_atom/2} \RefSP{read-term-from-atom/3} to avoid to add a
terminal dot at the end of the atom. The default value is \texttt{dot}.

\end{itemize}

\IdxPBD[read/2]{read(SorA, Term)} is equivalent to
\texttt{read\_term(SorA, Term, [])}.

\IdxPBD{read\_term/2} and \IdxPBD{read/1} apply to the current input stream.
...
as a syntax error. The read options \texttt{syntax\_error/1} and
\texttt{end\_of\_term/1} are GNU Prolog extensions.

\subsubsection{\texttt{read\_atom/2},
\texttt{read\_atom/1}, 
...
\IdxPBD[read\_atom/2]{read\_atom(SorA, Atom)} succeeds if \texttt{Atom}
unifies with the next atom read from the stream associated with the
stream-term or alias \texttt{SorA}.

\IdxPBD[read\_integer/2]{read\_integer(SorA, Integer)} succeeds if
\texttt{Integer} unifies with the next integer read from the stream
associated with the stream-term or alias \texttt{SorA}.

\IdxPBD[read\_number/2]{read\_number(SorA, Number)} succeeds if
...
\IdxPBD{read\_atom/1}, \IdxPBD{read\_integer/1} and \IdxPBD{read\_number/1}
...
\subsubsection{\texttt{read\_token/2},
\texttt{read\_token/1}}
\label{read-token/2}
...
\IdxPBD[read\_token/2]{read\_token(SorA, Token)} succeeds if \texttt{Token} unifies with
...
\item \IdxPTD[var]{var(A)}: a variable is read whose name is the atom
\texttt{A}.
...
\item \IdxPTD[string]{string(A)}: a string (double quoted item) is read whose
characters forms the atom \texttt{A}. 

\item \IdxPTD[punct]{punct(P)}: a punctuation character \texttt{P} is read
(\texttt{P} is a one-character atom in \texttt{()[]{\lb}|{\rb}}, the atom
\texttt{full\_stop} or the atom \texttt{end\_of\_file}).

\item \IdxPTD[back\_quotes]{back\_quotes(A)}: a back quoted item is read
whose characters forms the atom \texttt{A}.

\item \IdxPTD[extended]{extended(A)}: an extended character \texttt{A} (an
atom) is read.

\end{itemize}

As for \texttt{read\_term/3}, the behavior of \texttt{read\_token/2} can be
affected by some \Idx[Prolog flag]{Prolog flags} \RefSP{Term-input/output}.

\IdxPBD[read\_token/1]{read\_token/1} applies to the current input stream.
...
\subsubsection{\texttt{syntax\_error\_info/4}}
\label{syntax-error-info/4}
...
\IdxPBD[syntax\_error\_info/4]{syntax\_error\_info(FileName, Line, Column, Error)} returns the
information associated to the last syntax error. \texttt{Line} is the line
...
\IdxPBD[last\_read\_start\_line\_column/2]{last\_read\_start\_line\_column(Line, Column)} unifies \texttt{Line}
and \texttt{Column} with the line number and the column number associated to
...
\IdxPBD[write\_term/3]{write\_term(SorA, Term, Options)} writes
...
\item \IdxPOD[quoted]{quoted(true}/\texttt{false)}: if \texttt{true} each
...
\item \IdxPOD[ignore\_ops]{ignore\_ops(true}/\texttt{false}): if
...
\item \IdxPOD[numbervars]{numbervars(true}/\texttt{false}): if
...
\item \IdxPOD[namevars]{namevars(true}/\texttt{false}): if \texttt{true} a
...
\item \IdxPOD[space\_args]{space\_args(true}/\texttt{false}): if
...
\item \IdxPOD[portrayed]{portrayed(true}/\texttt{false}): if \texttt{true}
...
\texttt{Term} is non-variable then it is passed to \IdxPBD{portray/1}. If
...
\item \IdxPOD[max\_depth]{max\_depth(N}): controls the depth of output for
compound terms. \texttt{N} is an integer specifying the depth. The output of
a term whose depth is greater then \texttt{N} gives rise to the output of
...
\IdxPBD[write/2]{write(SorA, Term)} is equivalent to
\texttt{write\_term(SorA, Term, [])}.

\IdxPBD[writeq/2]{writeq(SorA, Term)} is equivalent to
\texttt{write\_term(SorA, Term, [quoted(true)])}.

\IdxPBD[write\_canonical/2]{write\_canonical(SorA, Term)} is equivalent to
\IdxPBD[write\_term/2]{write\_term(SorA, Term, [quoted(true), \\
ignore\_ops(true), numbervars(false)])}.

\IdxPBD[display/2]{display(SorA, Term)} is equivalent to
\texttt{write\_term(SorA, Term, [ignore\_ops(true), \\
numbervars(false)])}.

\IdxPBD[print/2]{print(SorA, Term)} is equivalent to
\texttt{write\_term(SorA, Term, [numbervars(false), \\
portrayed(true)])}.

\IdxPBD{write\_term/2}, \IdxPBD{write/1}, \IdxPBD{writeq/1},
\IdxPBD{write\_canonical/1}, \IdxPBD{display/1} and \IdxPBD{print/1} apply
...
are GNU Prolog predicates. \texttt{namevars/1}, \texttt{space\_args/1},
\texttt{portrayed/1} and \texttt{max\_depth/1} options are GNU Prolog
extensions.

\subsubsection{\texttt{format/3},
\texttt{format/2}}
\label{format/3}
...
\IdxPBD[format/3]{format(SorA, Format, Arguments)} writes the \texttt{Format} string
...
\texttt{\~{}a} & atom & print the atom without quoting \\

\hline

\texttt{\~{}Nc} & character code & print the character associated to the
...
\IdxPBD[format/2]{format/2} applies to the current output stream.
...
\subsubsection{\texttt{portray\_clause/2},
\texttt{portray\_clause/1}}
\label{portray-clause/2}
...
\IdxPBD[portray\_clause/2]{portray\_clause(SorA, Clause)} pretty prints
...
\IdxPBD[portray\_clause/1]{portray\_clause/1} applies to the current output stream.
...
\IdxPBD[get\_print\_stream/1]{get\_print\_stream(Stream)} unifies \texttt{Stream} with the
stream-term associated to the output stream used by \IdxPB{print/2}
...
\IdxPBD[op/3]{op(Priority, OpSpecifier, Operator)} alters the operator table.
...
\ErrCond{\texttt{Operator} is \texttt{','} or an element of the
...
\subsubsection{\texttt{current\_op/3}}
...
\IdxPBD[current\_op/3]{current\_op(Priority, OpSpecifier, Operator)} succeeds if
...
\IdxPBD[char\_conversion/2]{char\_conversion(InChar, OutChar)} alters the character-conversion
...
\IdxPBD[current\_char\_conversion/2]{current\_char\_conversion(InChar, OutChar)} succeeds if the
...
\IdxPBD[read\_term\_from\_atom/3]{}%
...
\IdxPBD[read\_term\_from\_chars/3]{}%
...
\IdxPBD[read\_term\_from\_codes/3]{}%
...
\subsubsection{\texttt{write\_term\_to\_atom/3},
\texttt{write\_to\_atom/2},
\texttt{writeq\_to\_atom/2}, \\
\texttt{write\_canonical\_to\_atom/2},
\texttt{display\_to\_atom/2}, 
\texttt{print\_to\_atom/2}, \\
\texttt{format\_to\_atom/3}} 
...
\IdxPBD[write\_term\_to\_atom/3]{}%
...
\IdxPBD[write\_term\_to\_chars/3]{}%
...
\IdxPBD[write\_term\_to\_codes/3]{}%
...
\IdxPBD[see/1]{see(FileName)} sets the current input stream to \texttt{FileName}.
...
\IdxPBD[tell/1]{tell(FileName)} sets the current output stream to \texttt{FileName}.
...
\IdxPBD[append/1]{append(FileName)} like \texttt{tell/1} but
...
See errors associated to \texttt{open/4} \RefSP{open/4}.
...
\IdxPBD[seeing/1]{seeing(FileName)} succeeds if \texttt{FileName} unifies
...
\IdxPBD[telling/1]{telling(FileName)} succeeds if \texttt{FileName} unifies with the
...
\IdxPBD[seen/0]{seen} closes the current input, and resets it to
\IdxPK{user\_input}.

\IdxPBD[told/0]{told} closes the current output, and resets it to
\IdxPK{user\_output}.
...
\IdxPBD[get0/1]{get0(Code)} succeeds if \texttt{Code} unifies with the next
character code read from the current input stream. Thus it is equivalent to
\IdxPBD[get/1]{get\_code(Code)} \RefSP{get-char/2}.
...
\IdxPBD[skip/1]{skip(Code)} skips just past the next character code \texttt{Code}
...
\IdxPBD[put/1]{put(Code)} writes the character whose code is \texttt{Code} onto the
current output stream. It is equivalent to \texttt{put\_code(Code)}
\RefSP{put-char/2}.

\IdxPBD[tab/1]{tab(N)} writes \texttt{N} spaces onto the current output
...
\OneLine{\textrm{head} \IdxPKD[(--{\gt})/2]{--{\gt}} \textrm{body}.}
...
\IdxPBD[expand\_term/2]{expand\_term(Term1, Term2)} succeeds if
...
\IdxPBD[phrase/3]{phrase(Phrase, List, Remainder)} succeeds if the list
...
\IdxPBD[phrase/2]{phrase(Phrase, List)} is equivalent to
...
\IdxPBD[abort/0]{abort} aborts the current execution. If this execution was initiated
...
\IdxPBD[stop/0]{stop} stops the current execution. If this execution was initiated
under a \Idx{top-level} the control is given back to the
top-level. Otherwise, \texttt{stop/0} is equivalent to \texttt{halt(0)}
(see below).

\IdxPBD[top\_level/0]{top\_level} starts a new recursive \Idx{top-level} (including the
...
\IdxPBD[break/0]{break} invokes a recursive top-level (no banner is displayed). To
end this new level simply type the end-of-file key sequence (\texttt{Ctl-D})
or its term representation: \texttt{end\_of\_file.}

\IdxPBD[halt/1]{halt(Status)} causes the GNU Prolog process to immediately exit back to
the shell with the return code \texttt{Status}.

\IdxPBD[halt/0]{halt} is equivalent to \texttt{halt(0)}.
...
\subsubsection{\texttt{once/1}, \texttt{({\bs}+)/1} - not provable,
\texttt{call\_with\_args/1-11}, \texttt{call/2}}

\begin{TemplatesOneCol}
once(+callable\_term)\\
{\bs}+(+callable\_term) \\
call\_with\_args(+atom, +term,\ldots, +term)\\
call(+callable\_term, ?boolean)

\end{TemplatesOneCol}

\Description

\IdxPBD[once/1]{once(Goal)} succeeds if \texttt{call(Goal)} succeeds. However
...
\IdxPBD[({\bs}+)/1]{{\bs}+ Goal} succeeds if \texttt{call(Goal)} fails and fails
otherwise. This built-in predicate gives negation by failure.

\IdxPBD[call\_with\_args/1-11]{call\_with\_args(Functor, Arg1,\ldots, ArgN)} calls the goal whose functor is \texttt{Functor} and whose arguments are \texttt{Arg1},\ldots, \texttt{ArgN} ($0 \leq \texttt{N} \leq 10$).

\IdxPBD[call/2]{call(Goal, Deterministic)} succeeds if \texttt{call(Goal)}
...
\texttt{once/1} and \texttt{({\bs}+)/1} are ISO predicates, \texttt{call\_with\_args/1-11} and \texttt{call/2} are GNU Prolog predicates.
...
\IdxPBD[repeat/0]{repeat} generates an infinite sequence of backtracking choices. The
...
\IdxPBD[for/3]{for(Counter, Lower, Upper)} generates an sequence of backtracking
...
\IdxPBD[atom\_length/2]{atom\_length(Atom, Length)} succeeds if \texttt{Length} unifies with
...
\IdxPBD[atom\_concat/3]{atom\_concat(Atom1, Atom2, Atom12)} succeeds if the name of
...
\IdxPBD[sub\_atom/5]{sub\_atom(Atom, Before, Length, After, SubAtom)} succeeds if atom
...
\subsubsection{\texttt{char\_code/2}}
\label{char-code/2}
...
\IdxPBD[char\_code/2]{char\_code(Char, Code)} succeeds if the character code for the
...
\IdxPBD[lower\_upper/2]{lower\_upper(Char1, Char2)} succeeds if \texttt{Char1} and
...
\IdxPBD[atom\_chars/2]{atom\_chars(Atom, Chars)} succeeds if \texttt{Chars}
is the list of one-char atoms whose names are the successive characters of
the name of \texttt{Atom}.

\IdxPBD[atom\_codes/2]{atom\_codes(Atom, Codes)} is similar to
...
\ErrCond{\texttt{Atom} is a variable and an element \texttt{E} of the list
\texttt{Chars} is neither a variable nor a one-char atom}
\ErrTerm{type\_error(character, E)}

\ErrCond{\texttt{Atom} is a variable and an element \texttt{E} of the list
\texttt{Codes} is neither a variable nor an integer}
\ErrTerm{type\_error(integer, E)}

\ErrCond{\texttt{Atom} is a variable and an element \texttt{E} of the list
\texttt{Codes} is an integer but not a character code}
\ErrTerm{representation\_error(character\_code)}
...
\IdxPBD[number\_atom/2]{number\_atom(Number, Atom)} succeeds if
\texttt{Atom} is an atom whose name corresponds to the characters of
\texttt{Number}.

\IdxPBD[number\_chars/2]{number\_chars(Number, Chars)} is similar to
\texttt{number\_atom/2} but deals with a list of character codes.

\IdxPBD[number\_codes/2]{number\_codes(Number, Codes)} is similar to
...
\ErrCond{\texttt{Number} is a variable and \texttt{Chars} is neither a list
nor a partial list}
\ErrTerm{type\_error(list, Chars)}

\ErrCond{\texttt{Number} is a variable and \texttt{Codes} is neither a list
nor a partial list}
\ErrTerm{type\_error(list, Codes)}

\ErrCond{\texttt{Number} is a variable and an element \texttt{E} of the list
\texttt{Chars} is neither a variable nor a one-char atom}
\ErrTerm{type\_error(character, E)}
...
\IdxPBD[name/2]{name(Constant, Codes)} succeeds if \texttt{Codes} is a list whose
...
\IdxPBD[atom\_hash/2]{atom\_hash(Atom, Hash)} succeeds if \texttt{Hash} is the internal
key associated to \texttt{Atom} (an existing atom). The internal key of an
atom is a unique integer $\geq$ 0 and $<$ to the \IdxPF{max\_atom}
\Idx{Prolog flag} \RefSP{set-prolog-flag/2}.

\begin{PlErrors}

\ErrCond{\texttt{Atom} and \texttt{Hash} are both variables}
\ErrTerm{instantiation\_error}
...
\ErrCond{\texttt{Hash} is an integer $<$ 0}
\ErrTerm{domain\_error(not\_less\_than\_zero, Hash)}
...
\IdxPBD[new\_atom/3]{new\_atom(Prefix, Hash, Atom)} unifies \texttt{Atom} with a new atom
whose name begins with the characters of the name of \texttt{Prefix} and
whose internal key is \texttt{Hash} \RefSP{atom-hash/2}. This predicate is
...
\IdxPBD[new\_atom/2]{new\_atom/2} is similar to \texttt{new\_atom/3}, but the atom
generated can have any (free) internal key.

\IdxPBD[new\_atom/1]{new\_atom/1} is similar to \texttt{new\_atom(atom\_, Atom)}, i.e.
...
\ErrCond{\texttt{Hash} is a variable}
\ErrTerm{instantiation\_error}
...
\ErrTerm{type\_error(variable, Atom)}
...
\IdxPBD[current\_atom/1]{current\_atom(Atom)} succeeds if there exists an atom that unifies
...
\IdxPBD[atom\_property/2]{atom\_property(Atom, Property)} succeeds if
...
\item \IdxPPD[length]{length(Length)}: \texttt{Length} is the length of the
name of the atom.

\item \IdxPPD[hash]{hash(Hash)}: \texttt{Hash} is the internal key of the
atom \RefSP{atom-hash/2}.
...
\IdxPBD[append/3]{append(List1, List2, List12)} succeeds if the concatenation of the
...
\IdxPBD[member/2]{member(Element, List)} succeeds if \texttt{Element} belongs to the
\texttt{List}. This predicate is re-executable on backtracking and can be
thus used to enumerate the elements of \texttt{List}.

\IdxPBD[memberchk/2]{memberchk/2} is similar to \texttt{member/2} but only succeeds once.
...
\IdxPBD[reverse/2]{reverse(List1, List2)} succeeds if \texttt{List2} unifies with the
...
\IdxPBD[delete/3]{delete(List1, Element, List2)} removes all occurrences of
\texttt{Element} in \texttt{List1} to provide \texttt{List2}. A strict term
equality is required, cf. \IdxPB{(==)/2} \RefSP{(==)/2}.

\IdxPBD[select/3]{select(Element, List1, List2)} removes one occurrence of
...
\IdxPBD[permutation/2]{permutation(List1, List2)} succeeds if \texttt{List2}
...
\IdxPBD[prefix/2]{prefix(Prefix, List)} succeeds if \texttt{Prefix} is a prefix of
\texttt{List}. This predicate is re-executable on backtracking.

\IdxPBD[suffix/2]{suffix(Suffix, List)} succeeds if \texttt{Suffix} is a suffix of
\texttt{List}. This predicate is re-executable on backtracking.
...
\IdxPBD[sublist/2]{sublist(List1, List2)} succeeds if \texttt{List2} is a sub-list of
\texttt{List1}. This predicate is re-executable on backtracking.
...
\IdxPBD[last/2]{last(List, Element)} succeeds if \texttt{Element} is the last element
of \texttt{List}.
...
\IdxPBD[length/2]{length(List, Length)} succeeds if \texttt{Length} is the length of
\texttt{List}.
...
\IdxPBD[nth/3]{nth(N, List, Element)} succeeds if the \texttt{N}\emph{th}
...
\IdxPBD[min\_list/2]{min\_list(List, Min)} succeeds if \texttt{Min} is the
smallest number in \texttt{List}.

\IdxPBD[max\_list/2]{max\_list(List, Max)} succeeds if \texttt{Max} is the
largest number in \texttt{List}.

\IdxPBD[sum\_list/2]{sum\_list(List, Sum)} succeeds if \texttt{Sum} is the
...
\IdxPBD[sort/2]{sort(List1, List2)} succeeds if \texttt{List2} is the
sorted list corresponding to \texttt{List1} where duplicate elements are
merged.

\IdxPBD{sort0/2} is similar to \texttt{sort/2} except that duplicate elements
are not merged.

\IdxPBD[keysort/2]{keysort(List1, List2)} succeeds if \texttt{List2} is the
sorted list of \texttt{List1} according to the keys. The list \texttt{List1}
consists of items of the form \texttt{Key-Value}. These items are sorted
...
\IdxPBD{sort/1}, \IdxPBD{sort0/1} and \IdxPBD{keysort/1} are similar to
\texttt{sort/2}, \texttt{sort0/2} and \texttt{keysort/2} but achieve a sort
...
variables. There are 3 kinds of objects which can be associated to a global
variable:
...
The initial value of a global variable is the integer 0. A global variable
is referenced by a name (i.e. name = an atom) possibly indexed if it is an
array (i.e. name = a compound term). In the following, \Param{GVarName}
represents such a reference to a global variable and its syntax is as
follows:

\begin{Indentation}
\begin{tabular}{@{}lll}
\Param{GVarName} & ::= & \Param{atom} \\
                 &     & \Param{atom}\texttt{(}\Param{Index}\texttt{,}\ldots\texttt{,}\Param{Index}\texttt{)} \\
\Param{Index}    & ::= & \Param{integer} \\
                 &     & \Param{GVarName}
\end{tabular}
\end{Indentation}

When a \Param{GVarName} is used as an index, the value of this variable
must be an integer. Indexes range from 0 to \Param{Size}-1 if the array has
\Param{Size} elements. The space necessary for copies and arrays are
dynamically allocated and recovered as soon as possible. For instance, when
an atom is associated to a global variable whose current value is an array,
the space for this array is recovered (unless the assignment is to be undone
when backtracking occurs).

\SPart{Arrays}: the predicates \texttt{g\_assign/2}, \texttt{g\_assignb/2}
and \texttt{g\_link/2} \RefSP{g-assign/2} define an array when the
assigned value is a compound term with principal functor \IdxPGD{g\_array}.
Then an array is assigned to \Param{GVarName}. There are 3 forms for the
term \texttt{g\_array}:

\begin{itemize}

\item \texttt{g\_array(Size)}: if \texttt{Size} is an integer 
$>$ 0 then defines an array of \texttt{Size} elements which are all
initialized with the integer \texttt{0}.
...
arrays. Some examples are presented later \RefSP{Examples}. 
...
The compound term with principal functor \IdxPGD{g\_array\_extend} can be
used instead of \texttt{g\_array} to modify the structure of a (possibly)
existing array. In that case, the existing elements of the array are not
initialized.

When an array is read, a term of the form \texttt{g\_array([Elem0,...,
ElemSize-1])} is returned.
...
\IdxPBD[g\_assign/2]{g\_assign(GVarName, Value)} assigns a copy of the term
\texttt{Value} to \texttt{GVarName}. This assignment is not undone when
backtracking occurs.

\IdxPBD[g\_assignb/2]{g\_assignb/2} is similar to \texttt{g\_assign/2} but the assignment
is undone at backtracking.

\IdxPBD[g\_link/2]{g\_link(GVarName, Value)} makes a link between \texttt{GVarName} to
...
\ErrCond{\texttt{GVarName} is a compound term and a sub-argument \texttt{E}
is not a valid index \RefSP{Global-variables}}
\ErrTerm{domain\_error(g\_array\_index, E)}
...
\IdxPBD[g\_read/2]{g\_read(GVarName, Value)} unifies \texttt{Value} with the term
...
\IdxPBD[g\_array\_size/2]{g\_array\_size(GVarName, Value)} unifies \texttt{Size} with the
...
\SPart{A simple counter}: the following predicate defines a counter using a
global variable:
...
The query: \texttt{inc(c, X)} will succeed unifying \texttt{X} with
\texttt{0}, another call to \texttt{inc(a, Y)} will then unify \texttt{Y}
with \texttt{1}, and so on.
...
\texttt{testl(B)} will succeed unifying \texttt{B} with \texttt{f(12)}
...
\IdxPBD[set\_prolog\_flag/2]{set\_prolog\_flag(Flag, Value)} sets the value
...
\IdxPFD{max\_stream} & an integer & maximum number of streams & N \\
...
\IdxPBD[current\_prolog\_flag/2]{current\_prolog\_flag(Flag, Value)}
...
\IdxPBD[set\_bip\_name/2]{set\_bip\_name(Functor, Arity)} initializes the context of the error
...
\IdxPBD[current\_bip\_name/2]{current\_bip\_name(Functor, Arity)} succeeds if \texttt{Functor} and
...
\IdxPBD[write\_pl\_state\_file/1]{write\_pl\_state\_file(FileName)} writes onto \texttt{FileName} all
...
\item the value of \IdxPF{char\_conversion}, \IdxPF{double\_quotes} and
\IdxPF{singleton\_warning} \Idx[Prolog flag]{Prolog flags}
\RefSP{set-prolog-flag/2}.

\end{itemize}

\IdxPBD[read\_pl\_state\_file/1]{read\_pl\_state\_file(FileName)} reads (restores) from
...
\subsubsection{\texttt{consult/1},
\texttt{'.'/2} - program consult}
\label{consult/1}
...
\IdxPBD[consult/1]{consult(Files)} compiles and loads into memory each file
...
\texttt{[ File | Files ]}, i.e. \IdxPBD['.'/2]{'.'(File, Files)} is
...
\IdxPBD[load/1]{load(Files)} loads into memory each file of the list
...
\IdxPBD[listing/1]{listing(Pred)} lists the clauses of the consulted predicate whose
...
\IdxPBD[listing/0]{listing} lists all clauses of all consulted predicates.
...
\IdxPBD[statistics/0]{statistics} displays statistics about memory usage and run times.

\IdxPBD[statistics/2]{statistics(Key, Value)} unifies \texttt{Value} with the current
...
\IdxPBD[user\_time/1]{user\_time(Time)} unifies \texttt{Time} with the user
CPU time elapsed since the start of Prolog.

\IdxPBD[system\_time/1]{system\_time(Time)} unifies \texttt{Time} with the
system CPU time elapsed since the start of Prolog.

\IdxPBD[cpu\_time/1]{cpu\_time(Time)} unifies \texttt{Time} with the CPU
time (user + system) elapsed since the start of Prolog.

\IdxPBD[real\_time/1]{real\_time(Time)} unifies \texttt{Time} with the
...
\IdxPBD[set\_seed/1]{set\_seed(Seed)} reinitializes the random number generator seed with
\texttt{Seed}.

\IdxPBD[randomize/0]{randomize} reinitializes the random number generator. This
...
\IdxPBD[get\_seed/1]{get\_seed(Seed)} unifies \texttt{Seed} with the current random
...
\IdxPBD[random/1]{random(Number)} unifies \texttt{Number} with a random floating point
number such that 0.0 $\leq$ \texttt{Number} $<$ 1.0.

\begin{PlErrors}

\ErrCond{\texttt{Number} is not a variable}
\ErrTerm{type\_error(variable, Number)}
...
\IdxPBD[random/3]{random(Base, Max, Number)} unifies \texttt{Number} with a random
...
\IdxPBD[absolute\_file\_name/2]{absolute\_file\_name(File1, File2)} succeeds if \texttt{File2} is
the absolute pathname associated to the relative file name \texttt{File1}.
...
\subsubsection{\texttt{decompose\_file\_name/4}}
...
\IdxPBD[decompose\_file\_name/4]{decompose\_file\_name(File, Directory, Prefix, Suffix)} decomposes
...
\IdxPBD[prolog\_file\_name/2]{prolog\_file\_name(File1, File2)} unifies \texttt{File2} with the
Prolog file name associated to \texttt{File1}. More precisely \texttt{File2}
...
\IdxPBD[argument\_counter/1]{argument\_counter(Counter)} succeeds if \texttt{Counter} is the
number of arguments of the command-line. Since the first argument is always
the name of the running program, \texttt{Counter} is always $\geq$ 1.
...
\IdxPBD[argument\_value/2]{argument\_value(N, Arg)} succeeds if the \texttt{N}\emph{th}
...
\IdxPBD[argument\_list/1]{argument\_list(Args)} succeeds if \texttt{Args} unifies with the
list of atoms associated to each argument on the command-line other than the
...
\IdxPBD[environ/2]{environ(Name, Value)} succeeds if \texttt{Name} is the name of an
...
\IdxPBD[make\_directory/1]{make\_directory(PathName)} creates the directory whose pathname is
\texttt{PathName}. 

\IdxPBD[delete\_directory/1]{delete\_directory(PathName)} removes the directory whose pathname is
\texttt{PathName}.

\IdxPBD[change\_directory/1]{change\_directory(PathName)} sets the current directory to the
...
\IdxPBD[working\_directory/1]{working\_directory(PathName)} succeeds if \texttt{PathName} is the
...
\IdxPBD[directory\_files/2]{directory\_files(PathName, Files)} succeeds if \texttt{Files} is the
...
\subsubsection{\texttt{rename\_file/2}}
...
\IdxPBD[rename\_file/2]{rename\_file(PathName1, PathName2)} renames the file or directory
...
\subsubsection{\texttt{delete\_file/1},
\texttt{unlink/1}}
...
\IdxPBD[delete\_file/1]{delete\_file(PathName)} removes the existing file whose pathname is
\texttt{PathName}.

\IdxPBD[unlink/1]{unlink/1} is similar to \texttt{delete\_file/1} except that it never
...
\IdxPBD[file\_permission/2]{file\_permission(PathName, Permission)} succeeds if
...
If \texttt{PathName} does not exists or if it its permissions do not include
\texttt{Permission} this predicate fails.

\IdxPBD[file\_exists/1]{file\_exists(PathName)} is equivalent to
...
\IdxPBD[file\_property/2]{file\_property(PathName, Property)} succeeds if \texttt{PathName} is
...
\item \IdxPPD[absolute\_file\_name]{absolute\_file\_name(File)}:
\texttt{File} is the absolute file name of \texttt{PathName}
\RefSP{absolute-file-name/2}.

\item \IdxPPD[real\_file\_name]{real\_file\_name(File)}: \texttt{File} is
the real file name of \texttt{PathName} (follows symbolic links).

\item \IdxPPD[type]{type(Type)}: \texttt{Type} is the type of
...
\item \IdxPPD[size]{size(Size)}: \texttt{Size} is the size (in bytes) of
\texttt{PathName}.

\item \IdxPPD[permission]{permission(Permission)}: \texttt{Permission} is a
permission of \texttt{PathName} \RefSP{file-permission/2}.

\item \IdxPPD[last\_modification]{last\_modification(DT)}: \texttt{DT} is
...
\subsubsection{\texttt{temporary\_name/2}}
...
\IdxPBD[temporary\_name/2]{temporary\_name(Template, PathName)} creates a unique file name
...
\IdxPBD[temporary\_file/3]{temporary\_file(Directory, Prefix, PathName)} creates a unique file
...
\subsubsection{\texttt{date\_time/1}}
\label{date-time/1}
...
\IdxPBD[date\_time/1]{date\_time(DateTime)} unifies \texttt{DateTime} with a compound term
...
\IdxPBD[host\_name/1]{host\_name(HostName)} unifies \texttt{HostName} with the name of the
...
\IdxPBD[os\_version/1]{os\_version(OSVersion)} unifies \texttt{OSVersion} with the
...
\IdxPBD[architecture/1]{architecture(Architecture)} unifies \texttt{Architecture} with the
...
\IdxPBD[shell/2]{shell(Command, Status)} invokes a new shell (named by the
...
\IdxPBD[shell/1]{shell(Command)} is equivalent to \texttt{shell(Command, 0)}.

\IdxPBD[shell/0]{shell} is equivalent to \texttt{shell('', 0)}.
...
\IdxPBD[system/2]{system(Command, Status)} invokes a new default shell passing
...
\IdxPBD[spawn/3]{spawn(Command, Arguments, Status)} executes \texttt{Command} passing
...
\IdxPBD[spawn/2]{spawn(Command, Arguments)} is equivalent to \texttt{spawn(Command,
Arguments, 0)}.
...
\IdxPBD[popen/3]{popen(Command, Mode, Stream)} invokes a new default shell (by
...
\IdxPBD[exec/5]{exec(Command, StreamIn, StreamOut, StreamErr, Pid)} invokes a new
...
with the stream-term associated to the standard input stream of
\texttt{Command} (it is an output stream). \texttt{StreamOut} is unified
with the stream-term associated to the standard output stream of
\texttt{Command} (it is an input stream). \texttt{StreamErr} is unified with
the stream-term associated to the standard error stream of \texttt{Command}
...
\IdxPBD[exec/4]{exec/4} is similar to \texttt{exec/5} but the process is removed
...
\ErrTerm{type\_error(variable, StreamIn)}

\ErrCond{\texttt{StreamOut} is not a variable}
\ErrTerm{type\_error(variable, StreamOut)}

\ErrCond{\texttt{StreamErr} is not a variable}
\ErrTerm{type\_error(variable, StreamErr)}

\ErrCond{\texttt{Pid} is not a variable}
\ErrTerm{type\_error(variable, Pid)}
...
\IdxPBD[wait/2]{wait(Pid, Status)} waits for the child process whose identifier is
...
\subsubsection{\texttt{prolog\_pid/1}}
...
\IdxPBD[prolog\_pid/1]{prolog\_pid(Pid)} unifies \texttt{Pid} with the process identifier
...
\IdxPBD[send\_signal/2]{send\_signal(Pid, Signal)} sends \texttt{Signal} to the process whose
...
\IdxPBD[sleep/1]{sleep(Seconds)} puts the GNU Prolog process to sleep for
...
\IdxPBD[select/5]{select(Reads, ReadyReads, Writes, ReadyWrites, TimeOut)} waits for a
...
\ErrCond{an element \texttt{E} of the \texttt{Reads} list is associated to
an output stream}
\ErrTerm{permission\_error(input, stream, E)}

\ErrCond{an element \texttt{E} of the \texttt{Writes} list is associated to
an input stream}
...
By default, streams associated to sockets are \texttt{block} buffered. The
...
\IdxPBD[socket/2]{socket(Domain, Socket)} creates a socket whose domain is
...
\ErrTerm{type\_error(variable, Socket)}
...
\subsubsection{\texttt{socket\_close/1}}
...
\IdxPBD[socket\_close/1]{socket\_close(Socket)} closes the socket whose descriptor is
\texttt{Socket}. This predicate should not be used if \texttt{Socket} has
given rise to a stream, e.g. by \IdxPB{socket\_connect/2}
...
\subsubsection{\texttt{socket\_bind/2}}
...
\IdxPBD[socket\_bind/2]{socket\_bind(Socket, Address)} binds the socket whose descriptor is
...
\IdxPBD[socket\_connect/4]{socket\_connect(Socket, Address, StreamIn, StreamOut)} connects the
...
unified with a stream-term associated to the input of the connection (it is
an input stream). Reading from this stream gets data from the socket.
\texttt{StreamOut} is unified with a stream-term associated to the output of
...
\subsubsection{\texttt{socket\_listen/2}}
...
\IdxPBD[socket\_listen/2]{socket\_listen(Socket, Length)} defines the socket whose descriptor
...
\IdxPBD[socket\_accept/4]{socket\_accept(Socket, Client, StreamIn, StreamOut)} extracts the
...
associated to the input of the connection (it is an input stream). Reading
from this stream gets data from the socket. \texttt{StreamOut} is unified
with a stream-term associated to the output of the connection (it is an
...
\IdxPBD[socket\_accept/3]{socket\_accept(Socket, StreamIn, StreamOut)} is equivalent to
...
\ErrTerm{type\_error(variable, Client)}
...
\subsubsection{\texttt{hostname\_address/2}}
...
\IdxPBD[hostname\_address/2]{hostname\_address(HostName, HostAddress)} succeeds if the Internet
...
\IdxPBD[get\_linedit\_prompt/1]{get\_linedit\_prompt(Prompt)} succeeds if \texttt{Prompt} is the
...
\IdxPBD[set\_linedit\_prompt/1]{set\_linedit\_prompt(Prompt)} sets the current \IdxK{linedit}
...
\IdxPBD[add\_linedit\_completion/1]{add\_linedit\_completion(Word)} adds \texttt{Word} in the list of
...
\IdxPBD[find\_linedit\_completion/2]{find\_linedit\_completion(Prefix, Word)} succeeds if \texttt{Word}
