...
\texttt{List}. If the \IdxPF{strict\_iso} \Idx{Prolog flag} is switched
off \RefSP{set-prolog-flag/2}, \texttt{length(L, L)} and friends fail (instead of looping).
...
\item the behavior of \IdxPB{length/2} is affected \RefSP{length/2}. It emits a \texttt{type\_error}
  if the first argument is not a list. It fails on cases like \texttt{length(L, L)} instead of looping.
\end{itemize}
