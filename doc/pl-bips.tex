...
\subsubsection{\texttt{acyclic\_term/1}}
   \AddPBD{acyclic\_term/1}
...
cyclic (sub-)term. This predicate is provided for compatibility only. GNU
Prolog does not support cyclic terms so the use of cyclic terms is not safe
and the resulting behavior is undefined (most often this leads to an infinite
loop and/or a system crash).
...
\textit{eval}(\texttt{E2})$\rfloor$ & I, I $\rightarrow$ I & N \\
...
\ErrCond{for \texttt{keysort/2}: an element \texttt{E} of \texttt{List2} is neither a varible nor a pair}
\ErrTerm{type\_error(pair, E)}
...
\item a callable term can be given as a predicate indicator (used as prototype).

\item built-in predicates are found by \texttt{current\_predicate/1}
\RefSP{current-predicate/1}.

\item the term parser (\texttt{read/1} and friends) is more indulgent (0'' is accepted and returns 39).
