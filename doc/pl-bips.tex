...
\SPart{Format control sequences}: the general format of a control
sequence is \texttt{\~{}NC}. The character \texttt{C} determines the
type of the control sequence. \texttt{N} is an optional numeric
argument (must evaluate to a positive integer integer). An alternative
form of \texttt{N} is \texttt{*}. A \texttt{*} implies that the next
argument \texttt{Arg} in \texttt{Arguments} should be used as a
numeric argument in the control sequence.
