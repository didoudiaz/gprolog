...
to define a specific transformation. If this predicate succeeds, \texttt{Term2} is
...
\texttt{expand\_term/2} on each \texttt{Term1} read in. In version prior to 1.6.0,
only DCG transformation was done by the compiler (i.e. \texttt{term\_expansion/2}
could not be used). Since 1.6.0, \texttt{term\_expansion/2} is supported.
On the command line, \IdxK{gplc} \RefSP{Using-the-compiler} simply compiles
the Prolog file, without loading it. As a result, \texttt{term\_expansion/2} cannot
execute user Prolog code even if defined in the source (the compiler is not aware
of its existence since it is not loaded). An initialization file can then be supplied, via
the \IdxK{--pre-load} option. Under the top-level, when a file
is consulted, the same command-line compiler is currently used (this may be
changed in the future). Some parameters of the top-level are already passed to
the compiler in an initialization file (such as operator definitions and certain
Prolog flags). In addition, the \texttt{term\_expansion/2} clauses known by the
top-level are also passed. In any case, the user can pass a custom initialization
file via \texttt{consult/2} \RefSP{consult/1}.
...
\IdxPFD{show\_information} & \texttt{\underline{on}} / \texttt{off} & show informational messages ? & N \\
...
\subsubsection{\IdxPBD{write\_pre\_load\_file/1},\label{write-pre-load-file/1}
               \IdxPBD{read\_pre\_load\_file/1}}

\begin{TemplatesOneCol}
write\_pre\_load\_file(+source\_sink)\\
read\_pre\_load\_file(+source\_sink)

\end{TemplatesOneCol}

\Description

\texttt{write\_pre\_load\_file(FileName)} writes onto \texttt{FileName} all
information that influences the parsing of a term \RefSP{Term-input/output}.
This allows a sub-process written in Prolog to read thisfile and then process
any Prolog term as done by the parent process. This is the case of the Prolog
to WAM compiler with the \IdxK{--pre-load} option
\RefSP{Using-the-compiler}.  The following elements are saved:
...
\item existing user dynamic clauses for \IdxPB{term\_expansion/2}.

\end{itemize}

\texttt{read\_pre\_load\_file(FileName)} reads and sets the Prolog
internal state from \texttt{FileName}. It is mainly used by the Prolog
to WAM compiler to set its read environment for parsing. If
\texttt{FileName} does not exist, it is computed using the predicate
\IdxPB{prolog\_file\_name/2} \RefSP{prolog-file-name/2}. Then
\IdxPB{absolute\_file\_name/2} \RefSP{absolute-file-name/2} applies.


\SPart{Prolog pre-load file}: prior to version 1.6.0, this file was
called ``prolog state'' file (pl-state) and was a binary file. Since
1.6.0, it is a (simplified) Prolog text file containing directives and
clauses which is treated by \texttt{read\_pre\_load\_file/1} as
follows:

\begin{itemize}
\item a \texttt{initialization/1} directive is directly executed.
\item any other directive term \texttt{Term} (i.e. \texttt{Term} is of the
  form \texttt{:- D}), is passed to predicate \texttt{add\_input\_term(Term,
    FileName, Line1, Line2)} if defined else it is ignored (\texttt{Line1}
  and \texttt{Line2} are begin and end line numbers of the directive in the
  state file).  In the case of the Prolog compiler, the predicate
  \texttt{add\_input\_term} adds the term at the beginning of the stream of
  terms to compile (it will be treated before the Prolog file to be
  compiled).
\item all clauses are directly asserted (useful to define
  \IdxPB{term\_expansion/2} and compagnion predicates).
...
\item \AddPOD{pre\_load}\texttt{pre\_load(File)}: use \texttt{File} as
  a Prolog pre-load file \RefSP{write-pre-load-file/1}.
