...
\subsubsection{\texttt{once/1}, \texttt{({\bs}+)/1} - not provable,
               \texttt{call/2-11}, 
	       \texttt{call\_with\_args/1-11}, \texttt{call\_det/2}}
   \AddPBD{once/1}
   \AddPBD{({\bs}+)/1}
   \AddPBD{call/2-11}
...
\texttt{once/1} and \texttt{({\bs}+)/1} are ISO predicates, \texttt{call/2-11}, \texttt{call\_with\_args/1-11} and \texttt{call\_det/2} are GNU Prolog predicates.
