...
\subsubsection{\IdxPBD{(==)/2} - term identical, \label{(==)/2}
   ...
   \AddPBD{("@<)/2} % Pb with @ in \index with HeVeA
   \AddPBD{("@=<)/2}
   \AddPBD{("@>)/2}
   \AddPBD{("@>=)/2}
...
\texttt{@>} and \texttt{@>=} are predefined infix operators \RefSP{op/3:(Term-input/output)}.
...
\ErrCond{\texttt{Order} is an atom but not \texttt{<}, \texttt{=} or \texttt{>}}
...
\subsubsection{\IdxPBD{setarg/4}, 
               \IdxPBD{setarg/3}, \IdxPBD{nb\_setarg/3},}
...
\subsubsection{\IdxPBD{current\_evaluable/1}\label{current-evaluable/1}}
...
\ErrCond{\texttt{EI} is neither a variable nor a predicate indicator}
\ErrTerm{type\_error(predicate\_indicator, EI)}

\ErrCond{\texttt{EI} is a term \texttt{Name/Arity} and \texttt{Arity} is
neither a variable nor an integer}
\ErrTerm{type\_error(integer, Arity)}

\ErrCond{\texttt{EI} is a term \texttt{Name/Arity} and \texttt{Name} is
...
\subsubsection{\IdxPBD{evaluable\_property/2}\label{evaluable-property/2}}
...
\SPart{Evaluable properties}:

\begin{itemize}

\item \IdxPPD{built\_in}: if the evaluable functor is built-in.

\item \IdxPPD{foreign}: if the evaluable functor is defined via the foreign
  interface (for future use).

\item \IdxPPD{static}: if the evaluable functor is static.

\item \IdxPPD{dynamic}: if the evaluable functor is dynamic (for future use).

\item \IdxPPD{iso}: if the evaluable functor is ISO.

\item \IdxPPD{template(Args, Ret)}: a specification for both the type of
...
\ErrCond{\texttt{Evaluable} is neither a variable nor a callable term}
\ErrTerm{type\_error(callable, Evaluable)}

\ErrCond{\texttt{Property} is neither a variable nor an evaluable property term}
\ErrTerm{domain\_error(evaluable\_property, Property)}
...
\subsubsection{\IdxPBD{(=:=)/2} - arithmetic equal, \label{(=:=)/2}
               ...
               \IdxPBD{(<)/2} - arithmetic less than,
               \IdxPBD{(=<)/2} - arithmetic less than or equal to, \\
               \IdxPBD{(>)/2} - arithmetic greater than,
               \IdxPBD{(>=)/2} - arithmetic greater than or equal to}
...
\ErrCond{\IdxPF{bounded} \Idx{Prolog flag} is true and \texttt{X} is
  \emph{maxint} and \texttt{Y} is a variable}
\ErrTerm{evaluation\_error(int\_overflow)}
...
are not found except if the \IdxPF{strict\_iso} \Idx{Prolog flag} is switched
off \RefSP{set-prolog-flag/2}. This predicate is re-executable on
backtracking.
...
\subsubsection{\IdxPBD{findall/4}, \IdxPBD{findall/3}}
...
\ErrCond{\texttt{Tail} is neither a partial list nor a list}
\ErrTerm{type\_error(list, Tail)}
...
can be done at open time or later using \IdxPB{set\_stream\_alias/2}
\RefSP{set-stream-alias/2}. Such an association automatically ends when the
...
\SPart{Current streams}: during execution there is a current input
...
built-in predicates \IdxPB{set\_input/1} \RefSP{set-input/1} and
\IdxPB{set\_output/1} \RefSP{set-output/1} can be used to change
them. These streams also have predefined aliases: \IdxPKD{current\_input}
and \IdxPKD{current\_output}. 

\SPart{Top-level and debugger aliases}: GNU Prolog provides predefined
aliases for the streams used by the top-level
(\IdxPKD{top\_level\_input}, \IdxPKD{top\_level\_output}) and by the
debugger (\IdxPKD{debugger\_input}, \IdxPKD{debugger\_output}).
...
becomes the standard input stream \IdxPK{user\_input}. If
\texttt{SorA} is the current output stream the current output stream
becomes the standard output stream \IdxPK{user\_output}. More
...
standard stream~\RefSP{Introduction:(Streams)}.
...
\item \AddPPD{file\_no}\texttt{file\_no(FD)}: \texttt{FD} is the file descriptor
...
ISO predicate. The \texttt{buffering/1}, \texttt{file\_name/1},
\texttt{file\_no/1}, \texttt{mirror/1} properties are GNU Prolog
extensions.
...
\subsubsection{\IdxPBD{set\_stream\_alias/2}, \label{set-stream-alias/2}
               \IdxPBD{add\_stream\_alias/2}}

\begin{TemplatesOneCol}
set\_stream\_alias(+stream\_or\_alias, +atom)\\
...
\texttt{set\_stream\_alias(SorA, Alias)} (re)assignes \texttt{Alias} as an alias
to the stream associated with the stream-term or alias \texttt{SorA}.

\texttt{add\_stream\_alias(SorA, Alias)} is similar except that it
throws an error if the \texttt{Alias} is already in use (i.e. assigned to another stream).
...
\ErrCond{\texttt{Alias} is already associated with an open stream (for \texttt{add\_stream\_alias/2})}
...
% If \texttt{Name} is not atom or does not respect the syntax of 
% variable names the pair is ignored. 
...
\begin{TemplatesOneCol}
format(+stream\_or\_alias, +character\_list\_or\_character\_code\_list\_or\_atom,
+list)\\
format(+character\_list\_or\_character\_code\_list\_or\_atom, +list)

\end{TemplatesOneCol}

\Description

\texttt{format(SorA, Control, Arguments)} interprets the \texttt{Arguments}
according to the \texttt{Control} string and prints the result to the stream
associated with the stream-term or alias \texttt{SorA}.
\texttt{format/2} applies to the current output stream.

\texttt{Control} is a text (represented as atom, chars or codes), which may
contain control sequences of the form \texttt{\~{}NC}, where \texttt{C} is a
(single-character) format control option, and \texttt{N} is its optional
non-negative integer parameter. As an extension, the use of C \texttt{printf()}
formatting sequence is also recognized. It must be introduced by \texttt{\~{}\%}
(in versions prior to 1.6.0. a single \texttt{\%} was enough but this created
too many compatibility issues when writing portable code displaying a
\texttt{\%}). Any characters that are not part of a control sequence are written to the
stream. \texttt{N} can be omitted, or given in one of the following ways:

\begin{itemize}
\item as a sequence of decimal digits
\item as the character \texttt{*}, in which case \texttt{N} will be taken as the
  next argument from \texttt{Arguments} (which must evaluate to a non-negative integer)
\item as \texttt{`}\textit{character}, \texttt{N} is then the integer code of the given
  \textit{character} (only really useful for \texttt{\~{}t}).
\end{itemize}

If a format option accepts a parameter, but \texttt{N} is omitted, a default
value is assumed. If a format option does not accept a parameter, but \texttt{N}
is given, \texttt{N} is ignored.
...
\newline
                               ...
                               \texttt{printf("\%.Nf",Arg)}.
\newline
                                     Similarly for  \texttt{\~{}NF}, \texttt{\~{}Ne}, \texttt{\~{}NE},
                                     \texttt{\~{}Ng} and \texttt{\~{}NG}. \\

\hline

\texttt{\~{}Nd} & integer expression & print the argument as a decimal number,
                                         shifted right \texttt{N} decimal places.
                                         If \texttt{N} is 0 no decimal point is printed (default: 0). \\

\hline

\texttt{\~{}ND} & integer expression & same as\texttt{\~{}Nd} except
...
\texttt{\~{}Ns} & character list, character code list or atom & print exactly \texttt{N} characters
                                        (default: the length of the list). \\

\hline

\texttt{\~{}i} & term & ignore the next argument. \\
...
\texttt{\~{}\%F} & atom, integer or float expression & interface to the C \texttt{printf(3)}.
\newline
                                                       The printf format \texttt{F}
                                                       can include flags,
                                                       minimum and precision fields
                                                       (\texttt{*} is allowed) but not
                                                       length-modifiers.
\newline
                                                       The argument can be:
                                                       atom, chars, codes, integer or
                                                       float expression. \\

\hline

\texttt{\~{}N} & none & print a new-line characters if not at the beginning
                        of a line (default: 1). \\

\hline

\texttt{\~{}N|} & none & sets a column boundary at position \texttt{N} and moves
                         the cursor to that line position (default: current position). \\

\hline
  
\texttt{\~{}N+} & none & sets a column boundary at \texttt{N} positions past the
                         previous column boundary and moves the cursor to that line
                         position (default: 8). \\

\hline

\texttt{\~{}Nt} & none & defines a location where padding can be
                         inserted. If \texttt{N} is given, it is taken as the
                         character code to be used for the padding character
                         (default is 32 = space). \\

\hline
\end{tabular}

\SPart{Column boundaries and padding}. The control sequences \texttt{\~{}|},
\texttt{\~{}+} and \texttt{\~{}t} manage column boundaries (tab stops) for text
alignment. Column boundaries are local to the current output line (and reset
when a newline is output). An implicit boundary always exists at column 0 (start
of line). Additional column boundaries may be specified either at an absolute
position using \texttt{\~{}N|} or relative to the previous boundary using
\texttt{\~{}N+}.

Padding is inserted between two successive column boundaries. The total padding
required to reach the next boundary is distributed across all padding points
defined by \texttt{\~{}t} that appear between those boundaries. Distribution is
as even as possible: earlier \texttt{\~{}t} s never receive more padding than
later ones, and the difference is at most one character. If no padding points
are present, all padding is inserted immediately before the right
boundary. Examples:
...
\ErrCond{an element \texttt{E} of \texttt{Format} is not a valid format control
sequence}
\ErrTerm{domain\_error(format\_control\_sequence, E)}

\ErrCond{an element of the \texttt{Format} is of the form \texttt{\~{}Nr} or
  \texttt{\~{}NR} but the radix \texttt{N} is not a valid radix}
\ErrTerm{domain\_error(format\_control\_sequence, N)}

\ErrCond{the \texttt{Arguments} list does not contain sufficient elements}
\ErrTerm{domain\_error(non\_empty\_list, [])}

\ErrCond{the \texttt{Arguments} list contains more elements than necessary}
\ErrTerm{domain\_error(number\_of\_arguments, [])}
...
Since 1.6.0: follows \MyUrl{https://prolog-lang.org/ImplementersForum/0110-format.html}{PIP-0110} (non-standard format \texttt{~{}?} has been removed).
...
\subsubsection{\IdxPBD{read\_line\_to\_chars/2},
               \IdxPBD{read\_line\_to\_codes/2}}
...
\texttt{SorA}. \texttt{Chars} is a list of characters or \IdxK{end\_of\_file} if
...
Definite clause grammars (\Idx{DCG}) are a useful notation to express grammar rules.
...
\OneLine{\textrm{head} \AddPKD{(-->)/2}\texttt{-->} \textrm{body}.}
...
\OneLine{assign(X,Y) --> left(X), [:=], right(Y), [;].}
...
\OneLine{assign(X,Y) --> left(X), [:=], right(Y0), {\lb}Y is Y0 {\rb},
[;].}
...
\IdxK{pl2wam} options and directives can be necessary
\RefSP{pl2wam-description}. Basically, it is necesary to inform the Prolog to
WAM compiler that a predicate must be \Idx{embeded} in its internal state (so it
...
as embeded. This can be done with \IdxDi{compiler\_modes/1} directive. Here is
...
\subsubsection{\IdxPBD{false/0}, \IdxPBD{once/1}, \IdxPBD{({\bs}+)/1} - not provable,
               \IdxPBD{call/2-11},
               \IdxPBD{call\_with\_args/1-11}, \IdxPBD{call\_det/2}, \IdxPBD{call\_nth/2}, \IdxPBD{countall/2}, \IdxPBD{forall/2}}
...
\texttt{call\_nth(Goal, N)} succeeds if \texttt{Goal} succeeded for
the\texttt{N}th time. If \texttt{N} = 0, \texttt{call\_nth/2} simply fails
without testing the validity of its arguments. This predicate fails or only
succeeds once (deterministic).

\texttt{countall(Goal, N)} executes all solutions of \texttt{Goal} and
unifies \texttt{N} with this number of solutions. This predicate always
succeeds once.
...
\ErrCond{\texttt{Goal} (or \texttt{Condition} or \texttt{Action}) is neither a
  variable nor a callable term} \ErrTerm{type\_error(callable, Goal)}
...
\ErrCond{for \texttt{call/2-11} the resulting arity of \texttt{Goal} (arity of
  $\texttt{Closure} + \texttt{N}$) is an integer $>$ \texttt{max\_arity} flag
  \RefSP{set-prolog-flag/2}} \ErrTerm{representation\_error(max\_arity)}
...
\subsubsection{\IdxPBD{flatten/2}}
...
\ErrCond{\texttt{List} is neither a list nor a partial list and the
\IdxPF{strict\_iso} \Idx{Prolog flag} is switched off \RefSP{set-prolog-flag/2}}
\ErrTerm{type\_error(list, List)}
...
\IdxPFD{show\_banner} & \texttt{\underline{on}} / \texttt{off} & show GNU Prolog  banner ? & N \\

\hline

\IdxPFD{show\_information} & \texttt{silent} / \texttt{\underline{normal}} / \texttt{informational} & level of shown messages & N \\
...
\item the following arithmetic rounding functions: \texttt{ceiling}, \texttt{floor},
  \texttt{round}, \texttt{truncate} also accept integers \RefSP{Evaluation-of-an-arithmetic-expression}.

\item \IdxPB{length/2} emits a \texttt{type\_error} if its first argument is not a list \RefSP{length/2}.
\end{itemize}
...
\texttt{Arity} correspond to the context of the error \RefSP{General-format-and-error-context}
...
\subsubsection{\IdxPBD{consult/2},\label{consult/1}
               \IdxPBD{consult/1},
...
\SPart{Consult options}: \texttt{Options} is a list of consult options. If this
list contains contradictory options, the rightmost option is the one which
applies. Possible options are:

\begin{itemize}

\item \AddPOD{quiet}\texttt{quiet}: do not display compiler message. The
  default value is determined by the \IdxPF{show\_information}
  \Idx{Prolog flag}.

\item \AddPOD{include}\texttt{include(File)}: use \texttt{File} as
  a Prolog include file. This file is passed to the compiler via the
  \IdxK{--include} \RefSP{Using-the-compiler}.

\end{itemize}


\SPart{Implicit include file}: due to separate compilation
\texttt{consult/1-2} creates an include file which is passed to the
Prolog to WAM compiler \RefSP{pl2wam-description}. This is done by
calling \texttt{write\_default\_include\_file/1}
\RefSP{write-default-include-file/1}.

\SPart{Shebang support}: since version 1.4.0, with the introduction of
\Idx{shebang support}, \texttt{consult/1} ignores the first line of a Prolog
source file which directly begins with \texttt{\#}. See
\RefSP{Scripting-Prolog} for more information about shebang support and
\Idx{PrologScript}.
...
\ErrCond{an element \texttt{E} of the \texttt{Options} list is neither a
variable nor a valid write-option}
\ErrTerm{domain\_error(consult\_option, E)}
...
\subsubsection{\IdxPBD{write\_default\_include\_file/1}}\label{write-default-include-file/1}

\begin{TemplatesOneCol}
write\_default\_include\_file(+source\_sink)

\end{TemplatesOneCol}

\Description

\texttt{write\_default\_include\_file(FileName)} writes onto
\texttt{FileName} relevant information influencing the parsing of a
term \RefSP{Term-input/output} or its expansion
\RefSP{Term-expansion}. This allows a sub-process written in Prolog to
first process this file in order to later process any Prolog term as
would be done by the parent process. The following elements are saved
in the \texttt{FileName} surrounded by \IdxDi{compiler\_mode/1} directives:
...
\item the value of \IdxPF{char\_conversion}, \IdxPF{double\_quotes},
  \IdxPF{back\_quotes}, \IdxPF{strict\_iso}, \IdxPF{singleton\_warning},
  \IdxPF{suspicious\_warning}, \IdxPF{multifile\_warning} and
  \IdxPF{show\_information} \Idx{Prolog flag}s \RefSP{set-prolog-flag/2}.
...
\subsubsection{\IdxPBD{copy\_file/2}}

\begin{TemplatesOneCol}
copy\_file(+atom, +atom)
...
inside that directory). See \IdxPB{absolute\_file\_name/2} for
information about the syntax of \texttt{PathName1} and
\texttt{PathName2} \RefSP{absolute-file-name/2}.
...
\texttt{file\_no} property \RefSP{stream-property/2}).

For each stream it is also possible to pass \IdxK{null} to bind the
...
\RefSP{wait/2} to wait for the termination of the child process.  Until a
...
\ErrCond{\texttt{SorAIn} is neither a variable nor a stream-term or alias}
\ErrTerm{domain\_error(stream\_or\_alias, SorAIn)}

\ErrCond{\texttt{SorAOut} is neither a variable nor a stream-term or alias}
\ErrTerm{domain\_error(stream\_or\_alias, SorAOut)}

\ErrCond{\texttt{SorAErr} is neither a variable nor a stream-term or alias}
\ErrTerm{domain\_error(stream\_or\_alias, SorAErr)}

\ErrCond{\texttt{SorAIn} is not associated with an open stream}
\ErrTerm{existence\_error(stream, SorAIn)}

\ErrCond{\texttt{SorAIn} is an output stream}
\ErrTerm{permission\_error(input, stream, SorAIn)}

\ErrCond{\texttt{SorAOut} is not associated with an open stream}
\ErrTerm{existence\_error(stream, SorAOut)}

\ErrCond{\texttt{SorAIn} is an input stream}
\ErrTerm{permission\_error(output, stream, SorAOut)}

\ErrCond{\texttt{SorAErr} is not associated with an open stream}
\ErrTerm{existence\_error(stream, SorAErr)}

\ErrCond{\texttt{SorAErr} is an input stream}
\ErrTerm{permission\_error(output, stream, SorAErr)}
...
%  LocalWords:  rnd infty Expr1 Expr2 Expr1 succ Pred _fd findall findall rb
%  LocalWords:  bagof setof eof eof eof univ lsb msb popcount acos atanh
%  LocalWords:  acosh asinh
