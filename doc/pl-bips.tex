...
\label{var/1}
   ...
   \AddPBD{callable/1}
   \AddPBD{list/1}
   \AddPBD{partial\_list/1}
   \AddPBD{list\_or\_partial\_list/1}
...
\subsubsection{\texttt{unify\_with\_occurs\_check/2}}
   \AddPBD{unify\_with\_occurs\_check/2}
...
\label{name-singleton-vars/1}
   \AddPBD{name\_singleton\_vars/1}
...
\texttt{predicate\_property(Pred, Property)} succeeds if
...
\item \IdxPOD[type]{type(}\IdxPOD{text}/\IdxPOD{binary}\texttt{)}: specifies whether the
...
\item \IdxPOD[eof\_action]{eof\_action(error}/\texttt{eof\_code}/\texttt{reset)}:
...
\texttt{open(SourceSink, Mode, Stream, Options)} is equivalent to
...
\label{flush-output/1}
   \AddPBD{flush\_output/1}
   \AddPBD{flush\_output/0}
...
\subsubsection{\texttt{at\_end\_of\_stream/1},
   ...
   \AddPBD{at\_end\_of\_stream/1}
   \AddPBD{at\_end\_of\_stream/0}
...
\label{set-stream-position/2}
   \AddPBD{set\_stream\_position/2}
...
\subsubsection{\texttt{set\_stream\_line\_column/3}}
   \AddPBD{set\_stream\_line\_column/3}
...
\subsubsection{\texttt{set\_stream\_eof\_action/2}}
   \AddPBD{set\_stream\_eof\_action/2}
...
\label{set-stream-buffering/2}
   \AddPBD{set\_stream\_buffering/2}
...
\subsubsection{\texttt{open\_input\_atom\_stream/2},
   ...
   \AddPBD{open\_input\_atom\_stream/2}
   \AddPBD{open\_input\_chars\_stream/2}
   \AddPBD{open\_input\_codes\_stream/2}
...
\subsubsection{\texttt{close\_input\_atom\_stream/1},
   ...
   \AddPBD{close\_input\_atom\_stream/1}
   \AddPBD{close\_input\_chars\_stream/1}
   \AddPBD{close\_input\_codes\_stream/1}
...
\label{open-output-atom-stream/1}
   \AddPBD{open\_output\_atom\_stream/1}
   \AddPBD{open\_output\_chars\_stream/1}
   \AddPBD{open\_output\_codes\_stream/1}
...
\label{close-output-atom-stream/2}
   \AddPBD{close\_output\_atom\_stream/2}
   \AddPBD{close\_output\_chars\_stream/2}
   \AddPBD{close\_output\_codes\_stream/2}
...
\subsubsection{\texttt{get\_key/2},
   ...
   \AddPBD{get\_key/2}
   \AddPBD{get\_key/1}
   \AddPBD{get\_key\_no\_echo/2}
   \AddPBD{get\_key\_no\_echo/1}
...
\texttt{get\_key(Code, SorA)} succeeds if \texttt{Code} unifies with the
...
\subsubsection{\texttt{unget\_char/2},
               ...
               \texttt{unget\_code/2},
               \texttt{unget\_code/1}}
   \AddPBD{unget\_char/2}
   \AddPBD{unget\_code/2}
   \AddPBD{unget\_char/1}
   \AddPBD{unget\_code/1}
...
\subsubsection{\texttt{unget\_byte/2},
               \texttt{unget\_byte/1}}
   \AddPBD{unget\_byte/2}
   \AddPBD{unget\_byte/1}
...
\item \IdxPOD[syntax\_error]{syntax\_error(error}/\texttt{warning}/\texttt{fail)}:
...
\label{read-atom/2}
   \AddPBD{read\_atom/2}
   \AddPBD{read\_integer/2}
   \AddPBD{read\_number/2}
   \AddPBD{read\_atom/1}
   \AddPBD{read\_integer/1}
   \AddPBD{read\_number/1}
...
\subsubsection{\texttt{read\_token/2},
               \texttt{read\_token/1}}
\label{read-token/2}
   \AddPBD{read\_token/2}
   \AddPBD{read\_token/1}
...
\subsubsection{\texttt{last\_read\_start\_line\_column/2}}
   \AddPBD{last\_read\_start\_line\_column/2}
...
\subsubsection{\texttt{write\_term/3},
               ...
               \texttt{print/1}}
\label{write-term/3}
   \AddPBD{write\_term/3}
   \AddPBD{portray/1}
   \AddPBD{write/2}
   \AddPBD{writeq/2}
   \AddPBD{write\_canonical/2}
   \AddPBD{display/2}
   \AddPBD{print/2}
   \AddPBD{write\_term/2}
   \AddPBD{write/1}
   \AddPBD{writeq/1}
   \AddPBD{write\_canonical/1}
   \AddPBD{display/1}
   \AddPBD{print/1}
...
\item \IdxPOD[priority]{priority(N)}: specifies the starting priority 
...
\texttt{portrayed/1}, \texttt{max\_depth/1} and \texttt{priority/1}
...
\texttt{\~{}Na} & atom & print the atom without quoting. \texttt{N} is minimal number of characters to print using spaces on the rigth if needed (default: the length of the atom) \\
...
\label{portray-clause/2}
   \AddPBD{portray\_clause/2}
   \AddPBD{portray\_clause/1}
...
\label{current-char-conversion/2}
   \AddPBD{current\_char\_conversion/2}
...
\label{read-term-from-atom/3}
   \AddPBD{read\_term\_from\_atom/3}
   \AddPBD{read\_from\_atom/2}
   \AddPBD{read\_token\_from\_atom/2}
...
\subsubsection{\texttt{read\_term\_from\_chars/3},
   ...
   \AddPBD{read\_term\_from\_chars/3}
   \AddPBD{read\_from\_chars/2}
   \AddPBD{read\_token\_from\_chars/2}
...
\subsubsection{\texttt{read\_term\_from\_codes/3},
   ...
   \AddPBD{read\_term\_from\_codes/3}
   \AddPBD{read\_from\_codes/2}
   \AddPBD{read\_token\_from\_codes/2}
...
\subsubsection{\texttt{write\_term\_to\_atom/3},
   ...
   \AddPBD{write\_term\_to\_atom/3}
   \AddPBD{write\_to\_atom/2}
   \AddPBD{writeq\_to\_atom/2}
   \AddPBD{write\_canonical\_to\_atom/2}
   \AddPBD{display\_to\_atom/2}
   \AddPBD{print\_to\_atom/2}
   \AddPBD{format\_to\_atom/3}
...
\subsubsection{\texttt{write\_term\_to\_chars/3},
   ...
   \AddPBD{write\_term\_to\_chars/3}
   \AddPBD{write\_to\_chars/2}
   \AddPBD{writeq\_to\_chars/2}
   \AddPBD{write\_canonical\_to\_chars/2}
   \AddPBD{display\_to\_chars/2}
   \AddPBD{print\_to\_chars/2}
   \AddPBD{format\_to\_chars/3}
...
\subsubsection{\texttt{write\_term\_to\_codes/3},
   ...
   \AddPBD{write\_term\_to\_codes/3}
   \AddPBD{write\_to\_codes/2}
   \AddPBD{writeq\_to\_codes/2}
   \AddPBD{write\_canonical\_to\_codes/2}
   \AddPBD{display\_to\_codes/2}
   \AddPBD{print\_to\_codes/2}
   \AddPBD{format\_to\_codes/3}
...
\texttt{call\_with\_args(Functor, Arg1,\ldots, ArgN)} calls the goal whose functor is \texttt{Functor} and whose arguments are \texttt{Arg1},\ldots, \texttt{ArgN} ($0 \leq \texttt{N} \leq 10$).

\texttt{call(Goal, Deterministic)} succeeds if \texttt{call(Goal)}
...
\subsubsection{\texttt{atom\_chars/2},
               \texttt{atom\_codes/2}}
\label{atom-chars/2}
   \AddPBD{atom\_chars/2}
   \AddPBD{atom\_codes/2}
...
\label{number-atom/2}
   \AddPBD{number\_atom/2}
   \AddPBD{number\_chars/2}
   \AddPBD{number\_codes/2}
...
\texttt{sublist(List1, List2)} succeeds if \texttt{List2} is a sub-list of
...
\texttt{sort0/2} is similar to \texttt{sort/2} except that duplicate elements
are not merged.
...
\texttt{sort/1}, \texttt{sort0/1} and \texttt{keysort/1} are similar to
...
\subsubsection{\texttt{current\_prolog\_flag/2}}
   \AddPBD{current\_prolog\_flag/2}
...
\label{write-pl-state-file/1}
   \AddPBD{write\_pl\_state\_file/1}
   \AddPBD{read\_pl\_state\_file/1}
...
\item the value of \IdxPF{char\_conversion}, \IdxPF{double\_quotes}, \IdxPF{back\_quotes} and \IdxPF{singleton\_warning} \Idx{Prolog flag}s
\RefSP{set-prolog-flag/2}.
...
\subsubsection{\texttt{decompose\_file\_name/4}}
   \AddPBD{decompose\_file\_name/4}
...
\subsubsection{\texttt{make\_directory/1},
   ...
   \AddPBD{make\_directory/1}
   \AddPBD{delete\_directory/1}
   \AddPBD{change\_directory/1}
...
\label{file-permission/2}
   \AddPBD{file\_permission/2}
   \AddPBD{file\_exists/1}
...
\subsubsection{\texttt{socket\_accept/4},
   ...
   \AddPBD{socket\_accept/4}
   \AddPBD{socket\_accept/3}
...
\subsubsection{\texttt{add\_linedit\_completion/1}}
   \AddPBD{add\_linedit\_completion/1}
...
\subsubsection{\texttt{find\_linedit\_completion/2}}
   \AddPBD{find\_linedit\_completion/2}
