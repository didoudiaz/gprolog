...
\subsubsection{\texttt{term\_variables/2},
		\texttt{term\_variables/3}}
   \AddPBD{term\_variables/2}
   \AddPBD{term\_variables/3}
...
\item \IdxPPD[meta\_predicate]{meta\_predicate(Head)}: if the predicate is a
...
would have a postfix and an infix definition. Or \texttt{Operator} or an
element of the \texttt{Operator} list is \texttt{|} and it would have a
prefix or a postfix definition or its \texttt{Priority} would be $\leq$ 1100.}  
