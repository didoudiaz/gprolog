...
the \IdxK{--pl-state} option. Under the top-level, when a file
...
to WAM compiler with the \IdxK{--pl-state} option
...
\texttt{read\_pl\_state\_file(FileName)} reads and sets the Prolog state
from \texttt{FileName}. It is mainly used by the Prolog to WAM compiler to
set its read environment.

\SPart{Prolog state file}: prior to version 1.6.0, the state file was a
binary file produced by \texttt{write\_pl\_state\_file/1} exclusively. Since
1.6.0, it is a (simplified) Prolog text file containing directives and
clauses which are treated by \texttt{read\_pl\_state\_file/1} as follows:
...
\item \AddPOD{pl\_state}\texttt{pl\_state(File)}: use \texttt{File} as a Prolog
  state file \RefSP{write-pl-state-file/1}.
