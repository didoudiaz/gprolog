...
The GNU Prolog compiler \RefSP{The-GNU-Prolog-compiler} automatically
...
compilation, some \IdxK{pl2wam} options and directives can be
necessary \RefSP{pl2wam-description}. Basically, it is necesary to
inform the Prolog to WAM compiler that a predicate must be ``loaded''
in its internal state (so it can be executed during term
expansion). This can be done with \IdxDi{pragma/1} directives.
...
in the \texttt{FileName} using the \IdxDi{pragma/1} directive:
