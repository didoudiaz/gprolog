...
options and directives can be necessary, this is described in the
\IdxK{pl2wam} description \RefSP{pl2wam-description}.
...
\item \AddPOD{pre\_load}\texttt{pre\_load(File)}: use \texttt{File} as
  a Prolog pre-load file. This file is passed to the compiler via the
  \IdxK{--pre-load} \RefSP{Using-the-compiler}.

\end{itemize}


\SPart{Implicit pre-load file}: due to separate compilation
\texttt{consult/1-2} creates as pre-load file which is passed to the Prolog
to WAM compiler \RefSP{pl2wam-description}. This is done by calling
\texttt{write\_pre\_load\_file/1} \RefSP{write-pre-load-file/1}.
...
\texttt{write\_pre\_load\_file(FileName)} writes onto \texttt{FileName}
relevant information influencing the parsing of a term
\RefSP{Term-input/output} or its expansion \RefSP{Term-expansion}. This
allows a sub-process written in Prolog to first process this file in order to
later process any Prolog term as would be done by the parent process. The
following elements are saved in the \texttt{FileName}:
...
\item existing dynamic clauses defining \IdxPB{term\_expansion/2}.
