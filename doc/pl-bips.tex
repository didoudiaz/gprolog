...
\textit{eval}(\texttt{E2}) & IF, IF $\rightarrow$ ? & N \\
...
\item \IdxPOD[mirror]{mirror(Mirror)}: specifies the stream associated with
...
\ErrCond{an element \texttt{E} of the \texttt{Options} list is
\texttt{mirror(M)} and \texttt{M} iis an input stream}
...
\item \IdxPPD[mirror]{mirror(M)}: \texttt{M} is a mirror stream of the 
stream.
...
explicitely removed using \IdxPB{remove\_stream\_mirror/2}
\RefSP{remove-stream-mirror/2} or implicitely when \texttt{Mirror} is closed.
Several mirror streams can be associated to a same stream. If \texttt{Mirror}
...
\subsubsection{\texttt{remove\_stream\_mirror/2}}
\label{remove-stream-mirror/2}
   \AddPBD{remove\_stream\_mirror/2}
...
\begin{tabular}{ll}
\texttt{a} & the content of variable associated to \texttt{a} (any kind) \\
\texttt{t(1)} & the 2nd element of the array associated to \texttt{t} \\
\texttt{t(k)} & if the value associated to \texttt{k} is I, the Ith element of the array associated to \texttt{t} \\
\texttt{a-1-2} & if the value associated to \texttt{a} is \texttt{f(g(a,b,c),2)}, the sub-term \texttt{b} \\
\end{tabular}
...
\subsubsection{\texttt{g\_set\_bit/2},
   ...
   \AddPBD{g\_set\_bit/2}
   \AddPBD{g\_reset\_bit/2}
   \AddPBD{g\_test\_set\_bit/2}
   \AddPBD{g\_test\_reset\_bit/2}
...
0. In the parent process, the status of the child process can be ontained
...
associated to the input side of the pipe and \texttt{StreamOut} is unified
with the stream-term associated to output side. This predicate is an
