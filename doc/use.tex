...
\begin{Code}
\begin{verbatim}
GNU Prolog 1.6.0 (6.4 bits)
Compiled Jun  4 2023, 18:48:57 with gcc
...
\begin{CmdOptions}
\IdxKD{--pre-load} \Param{FILE} & include FILE at the beginning of the compilation \\
...
\IdxK{--pic} & produce position independent code (PIC) \\
...
intended to produce either native code or byte-code. It is written in GNU
Prolog and bootstrapped to produce a native executable that can be invoked
from the command line (e.g. by \texttt{gplc}). Currently, the Prolog file is
read and compiled, \emph{but not loaded} into the compiler (this may change
in the future). Exceptions are a few directives such as \IdxDi{op/3},
\IdxDi{char\_conversion/2} which are executed by \texttt{pl2wam}. Since
version 1.6.0, GNU prolog supports \IdxPB{term\_expansion/2}. This mechanism
may require the execution of user code during compilation. Various
directives, such as \IdxDi{load\_only/1} or \IdxDi{load\_also/1}, are avaible
to inform about predicates to be loaded into the compiler.

This kind of code can be part of the Prolog file to compile (directly or via
a \IdxDi{include/1} directive) or can be given in a separate file and passed
to the compiler with the \texttt{--pre-load} option. A pre-load file is a
Prolog text which is included by the compiler when started (as by the
\IdxDi{include/1} directive). Several \texttt{--pre-load} options can be
provided, they are considered in order of apparition.
...
as a command line subprocess (spawn). A pre-load file is created by
\texttt{consult/1} and passed to the compiler to to reproduce certain
...
files. Since 1.6.0, this has been replaced by pre-load files (simple Prolog
text files) and new directives (e.g. \texttt{load\_only}).  The built-in
predicate \IdxPB{consult/2} can be used to pass additional pre-load files.
