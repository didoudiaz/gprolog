...
\begin{CmdOptions}
\IdxKD{--pre-load} \Param{FILE} & pre-load FILE to set the internal compiler state \\
...
To execute the Prolog to WAM compiler in a given \emph{compiler environment}
(operator definitions, character conversion table,\ldots) use
\texttt{--pre-load} \Param{FILE} which immediately reads and ``loads'' the
specified file into the compiler via the \IdxPB{read\_pre\_load\_file/1}
\RefSP{write-pre-load-file/1}. Several \texttt{--pre-load} options can be
