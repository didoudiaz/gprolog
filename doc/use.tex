...
\OneLineTwoCols{\% gprolog}{(the \texttt{\%} symbol is the operating system
shell prompt)}

The top-level invocation does not need any supplemental line-command option.
When present these options can be retrieved using \IdxPB{argument\_value/2}
\RefSP{argument-value/2} or \IdxPB{argument\_list/1} \RefSP{argument-list/1}.
However, the \texttt{--verbose} and \texttt{--help} options are recognized
for compatibility purpose with other GNU programs.
...
\SkipLine
\One{X = [a]}
\Two{Y = [b,c] ?}{(here the user presses \texttt{a} to compute all remaining
solutions)}
\SkipLine
\One{X = [a,b]}
...
\texttt{local} & 2048 & \texttt{LOCALSZ} & control stack (environments
and choice-points) \\

\hline

\texttt{global} & 4096 & \texttt{GLOBALSZ} & heap (compound terms)
\\

\hline

\texttt{trail} & 2048 & \texttt{TRAILSZ} & conditional bindings (bindings
to undo at backtracking) \\

\hline

\texttt{cstr} & 2048 & \texttt{CSTRSZ} & finite domain constraint stack
...
instance to allocate 4096 Kb to the local stack under a Unix shell use:

\begin{CodeTwoCols}[6cm]
\Two{LOCALSZ=4096; export LOCALS}{(under \texttt{sh} or \texttt{bash})}
\Two{setenv LOCALSZ 4096}{(under \texttt{csh} or \texttt{tcsh})}
\end{CodeTwoCols}
...
called \Idx{mini-assembly} (\Idx{MA}). The mini-assembly file is then mapped
to the assembly language of the target machine. This assembly file is
