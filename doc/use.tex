...
\IdxKD{--no-gui-console} & disable Windows GUI console (run in text mode) \\

\IdxKD{--quiet} & suppress informational messages (e.g. top-level banner) \\
...
Compiled Nov 18 2025, 12:05:57 with gcc
...
Note that the banner (and other informative messages) can be removed
with the \IdxK{--quiet} option which turns off the
\IdxPF{show\_information} \Idx{Prolog flag}.
...
\SkipLine
\One{X = [a,b,c]}
\Two{Y = []}     {(last solution is reached)}
\SkipLine
\One{yes}
\end{CodeTwoCols}
...
The top-level uses variable binding predicates \RefSP{Variable-naming/numbering}.
To display the value of a variable, the top-level calls
...
\One{Y = 100001}
\SkipLine
\Two{(156 ms) yes}{(the query took 156ms of user time)}
\end{CodeTwoCols}
...
\texttt{Esc-<} & \texttt{Page Up} & recall first history line \\

\hline

\texttt{Esc->} & \texttt{Page Down} & recall last history line \\
...
\texttt{no}          & do not activated linedit (should be the only value of the variable) \\
...
\texttt{local} & 50000 & \texttt{LOCALSZ} & control stack (environments
and choice-points) \\

\hline

\texttt{global} & 100000 & \texttt{GLOBALSZ} & heap (compound terms) \\

\hline

\texttt{trail} & 50000 & \texttt{TRAILSZ} & conditional bindings (bindings
to undo at backtracking) \\

\hline

\texttt{cstr} & 50000 & \texttt{CSTRSZ} & finite domain constraint stack
(FD variables and constraints) \\

\hline

\texttt{atoms} & 100000 & \texttt{MAX\_ATOM} & atom table size \\
...
\Two{LOCALSZ=100000; export LOCALSZ}{(under \texttt{sh} or \texttt{bash})}
\Two{setenv LOCALSZ 100000}{(under \texttt{csh} or \texttt{tcsh})}
\end{CodeTwoCols}
...
\texttt{.pl}, \texttt{.pro}, \texttt{.prolog} & Prolog source file &  \IdxK{pl2wam} \\

\hline

\texttt{.wam} & WAM source file & \IdxK{wam2ma} \\

\hline

\texttt{.ma} & Mini-assembly source file & \IdxK{ma2asm} \\

\hline
 
\texttt{.s} & Assembly source file & the \Idx{assembler} \\
...
\texttt{.fd} & Finite Domain constraint source file & \IdxK{fd2c} \\

\hline

any other suffix (\texttt{.o}, \texttt{.a},\ldots) & any other type
(object, library,\ldots) & the linker (C \Idx{linker}) \\
...
\label{pl2wam-options}

\begin{CmdOptions}
\IdxKD{--include} \Param{FILE} & include FILE at the beginning of the compilation \\

\IdxKD{--wam-comment} \Param{COMMENT} & emit \Param{COMMENT} as a comment in the WAM file (can be repeated) \\
...
\IdxKD{--pic}, \IdxKD{--fPIC} & produce position independent code (PIC) (not yet fully implemented) \\
...
\IdxKD{-C} \Param{OPTION}, \IdxKD{-Wc,}\Param{OPTION} & pass \Param{OPTION} to the C compiler \\
...
\begin{CmdOptions}
\IdxKD{-A} \Param{OPTION}, \IdxKD{-Wa,}\Param{OPTION} & pass \Param{OPTION} to the assembler \\
...
\IdxKD{-L} \Param{OPTION}, \IdxKD{-Wl,}\Param{OPTION} & Pass \Param{OPTION} to the linker \\
...
input file names. For this reason, several meta characters are available for
substitution in \Param{FILE}:
...
\SPart{The Prolog to WAM compiler (\texttt{pl2wam})}\label{pl2wam-description}

The \IdxKD{pl2wam} (sub)compiler translates a Prolog program into WAM code
intended to produce either native code or byte-code. It is written in GNU Prolog
and bootstrapped to produce a native executable that can be invoked from the
command line (e.g. by \texttt{gplc}). Currently, the Prolog file is read and
compiled, \emph{but not loaded} into the compiler (this may change in the
future). Exceptions are a few directives such as \IdxDi{op/3},
\IdxDi{char\_conversion/2} which are executed by \texttt{pl2wam}. Since version
1.6.0, GNU prolog supports \IdxPB{term\_expansion/2}. This mechanism may require
the execution of user Prolog code during compilation. For this, the code must be
declared as \Idx{embeded} via the \IdxDi{compiler\_mode/1} directive. This user
code can be part of the Prolog file to compile (directly or via a
\IdxDi{include/1} directive) or can be given in a separate file and passed to
the compiler with the \texttt{--include} option. An include file is a Prolog
text which is included by the compiler when started (as by the \IdxDi{include/1}
directive). Several \texttt{--include} options can be provided, they are
considered in order of apparition.

By default, \texttt{pl2wam} runs in the native-code compilation scheme. To
generate a WAM file for byte-code use the \texttt{--wam-for-byte-code}
option. The resulting file can then be quickly loaded using \IdxPB{load/1}
\RefSP{load/1}. This is used by \IdxPB{consult/1} which invokes the compiler
as a command line subprocess (spawn). A default include file is created by
\texttt{consult/1-2} and passed to the compiler to reproduce certain
parameters of the top-level's internal state: operator definitions, Prolog
flag values, etc. \RefSP{consult/1}. Prior to version 1.6.0, another
mechanism was used, based on ``prolog state'' (pl-state) and stored in binary
files. Since 1.6.0, this has been replaced by include files (simple Prolog
text files) and new directives (e.g. \texttt{load\_only}). The built-in
predicate \IdxPB{consult/2} can be used to pass additional include files.
...
\SPart{Producing a native code executable}
...
\item the predicate cannot be consulted. This is the case of a predicate calling
  foreign code, like a predicate interfacing with C
  \RefSP{Interfacing-Prolog-and-C} or a predicate defining a new FD constraint.
...
To obtain a fully extended executable, it is desirable to accept the same set of
opions as the original top-level, see
\RefSP{The-GNU-Prolog-interactive-interpreter}, e.g. \IdxK{--init-goal}. For
this it is necessary to link \IdxK{main()} function used by the original
top-level. This can be achieved passing the \IdxK{--new-top-level} to
\texttt{gplc}:
...
[\Param{MODULE}\texttt{:}]\Param{PRED}\texttt{/}\Param{N} (where the
\Param{MODULE} can be omitted) will give rise to an identifier of the
following form:
...
\item \Param{K} is a digit in \texttt{0}..\texttt{5} storing coding information
  about \Param{MODULE} and \Param{PRED}. Possible values are:
...
\IdxKD{gplc-filter} command as follows:

\OneLineTwoCols[5.5cm]{\% gplc-filter \textrm{[}\Param{OPTION}\textrm{]\ldots}~\Param{FILE}\textrm{\ldots}}{(the \texttt{\%} symbol is the operating system shell prompt)}

\SPart{Options}:

\begin{CmdOptions}
\IdxKD{--demangling} or \IdxKD{--decode} & demangling  mode (this is the default mode) \\

\IdxKD{--mangling} or \IdxKD{--encode} & mangling mode \\

\IdxKD{--relax} & encode/decode also predicate names (not only predicate
indicators) \\

\IdxKD{--strict} & encode/decode only predicate indicators (default) \\

\IdxKD{--quote} & quote decoded predicate names (as done by writeq) \\

\IdxKD{--no-quote} & do not quote decoded predicate names \\
...
\IdxKD{-M} or \IdxKD{-H} & same as: \texttt{--cmd-line --encode --relax} \\

\IdxKD{-D} or \IdxKD{-P} & same as: \texttt{--cmd-line --decode --relax --quote} \\

\IdxKD{-h} or \IdxKD{--help} & print a help and exit \\
...
Without arguments \texttt{gplc-filter} runs in decoding mode reading its
standard input and decoding (demangling) each symbol corresponding to a predicate
indicator. To use \texttt{gplc-filter} in the encoding (mangling) mode the
\texttt{--encode} option must be specified. By default \texttt{gplc-filter} only
...
possible to force \texttt{gplc-filter} to decode an auxiliary predicate as its
...
If no file is specified, \texttt{gplc-filter} processes its standard input
otherwise each file is treated sequentially. Specifying the
\texttt{--cmd-line} option informs \texttt{gplc-filter} that each argument is not
...
\OneLine{\% gplc-filter -E \Param{PRED}}

NB: if \Param{PRED} is a complex atom it is necessary to quote it 
(the quotes must be passed to \texttt{gplc-filter}). Here is an example under bash:

\begin{Indentation}
\begin{verbatim}
% gplc-filter -E \'x+y=z\'/3
X1_782B793D7A__a3
\end{verbatim}
\end{Indentation}
