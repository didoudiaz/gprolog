...
may require the execution of user code during compilation. This can be
controlled via \IdxDi{pragma/1} directives. This user code can be part of the
Prolog file to compile (directly or via a \IdxDi{include/1} directive) or can
be given in a separate file and passed to the compiler with the
\texttt{--include} option. An include file is a Prolog text which is included
by the compiler when started (as by the \IdxDi{include/1} directive). Several
\texttt{--include} options can be provided, they are considered in order of
apparition.
