...
\begin{CmdOptions}
\IdxKD{--init-goal}  \Param{GOAL} & execute \Param{GOAL} before top\_level/0 \\
\IdxKD{--entry-goal} \Param{GOAL} & execute \Param{GOAL} inside top\_level/0 \\
\IdxKD{--query-goal} \Param{GOAL} & execute \Param{GOAL} as a query for top\_level/0 \\
...
it is encountered (while the commnad-line is processed). \Param{GOAL} is thus
...
executed in the oder of apparition. Thus, all init goals are executed (in the
order of apparition) before all entry goals and all entry goals are executed
...
To be executed, a \Param{GOAL} is transformed into a term using
\IdxPB[read\_term\_from\_atom/3]
{read\_term\_from\_atom(Goal, Term, [end\_of\_term(eof)])}. Respecting both
...
since it introduces an \Idx{escape sequence} both in sh and inside Prolog
...
controlled using the \IdxPF{back\_quotes} \Idx{Prolog flag} \RefSP{set-prolog-flag/2}).
...
\texttt{[\IdxPO[quoted]{quoted(true)},\IdxPO[numbervars]{numbervars(false)},
\IdxPO[namevars]{namevars(true)}]} \RefSP{write-term/3}. A term of the
...
associated to a new consulted predicate,\ldots). When the completion key
...
\texttt{local} & 4096 & \texttt{LOCALSZ} & control stack (environments
and choice-points) \\

\hline

\texttt{global} & 8192 & \texttt{GLOBALSZ} & heap (compound terms)
\\

\hline

\texttt{trail} & 3072 & \texttt{TRAILSZ} & conditional bindings (bindings
to undo at backtracking) \\

\hline

\texttt{cstr} & 3072 & \texttt{CSTRSZ} & finite domain constraint stack
...
\Two{LOCALSZ=8192; export LOCALS}{(under \texttt{sh} or \texttt{bash})}
\Two{setenv LOCALSZ 8192}{(under \texttt{csh} or \texttt{tcsh})}
\end{CodeTwoCols}
...
\IdxKD{--no-decode-hexa} & do not decode hexadecimal predicate names \\
...
\begin{CmdOptions}
\IdxKD{--pl-state} \Param{FILE} & read \Param{FILE} to set the
...
\IdxKD{--no-redef-error} & no not show errors for built-in predicate
redefinitions \\
...
\begin{CmdOptions}
\IdxKD{--c-compiler} \Param{FILE} & use \Param{FILE} as C compiler \\
...
named \Param{FILE}. If not specified, the output file name depends
on the last stage reached by the compiler. If the link is not done the
output file name(s) is the input file name(s) with the suffix associated to
the last stage. If the link is done, the name of the executable is the name
(without suffix) of the first file name encountered in the command-line.
Note that if the link is not done \texttt{-o} should be used if only one
...
explained in more detail later \RefSP{The-hexadecimal-predicate-name-encoding}. By default the error messages from the linker (e.g. multiple
...
prevents \IdxK{gplc} from filtering linker output messages and hexadecimal
...
name followed by the arity). The symbol associated to the predicate name is
...
So, from the \Idx{mini-assembly} stage, each predicate indicator is replaced
...
compiler has to decode the symbol associated to the predicate indicator. For
this \IdxK{gplc} filters each message emitted by the linker to locate and
decode eventual predicate indicators. This filtering can be deactivated
specifying \texttt{--no-decode-hexa} when invoking \IdxK{gplc} \RefSP{Using-the-compiler}.
...
\begin{CmdOptions}
\IdxKD{--encode} & encoding mode (default mode is decoding) \\
...
\IdxKD{-H} & same as: \texttt{--cmd-line --encode} \\

\IdxKD{-P} & same as: \texttt{--cmd-line --relax} \\
